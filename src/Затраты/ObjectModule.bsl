...
Процедура УстановитьНачальныеНастройки(ДополнительныеПараметры = Неопределено) Экспорт
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |			ТОГДА ЕСТЬNULL(ТребованиеНакладнаяМатериалы.Номенклатура, ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))
	               |		КОГДА ИсточникДанных.Регистратор ССЫЛКА Документ.АвансовыйОтчет
	               |			ТОГДА ЕСТЬNULL(ВЫРАЗИТЬ(АвансовыйОтчетПрочее.Содержание КАК СТРОКА(100)), """")
	               |		КОГДА ИсточникДанных.Регистратор ССЫЛКА Документ.ПоступлениеТоваровУслуг
	               |			ТОГДА ЕСТЬNULL(ВЫРАЗИТЬ(ПоступлениеТоваровУслуг.Содержание КАК СТРОКА(100)), """")
	               ...
	               |	ЭтоИнвестиции}
	               |ИЗ
	               |	РегистрНакопления.Затраты.Обороты(&ДатаНач, &ДатаКон, Запись, {(Подразделение).* КАК Подразделение, (СтатьяЗатрат).* КАК СтатьяЗатрат, (НоменклатурнаяГруппа).* КАК НоменклатурнаяГруппа, (Заказ).* КАК Заказ}) КАК ИсточникДанных
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ТребованиеНакладная.Материалы КАК ТребованиеНакладнаяМатериалы
	               |		ПО ИсточникДанных.Регистратор = ТребованиеНакладнаяМатериалы.Ссылка
	               |			И ИсточникДанных.НомерСтроки = ТребованиеНакладнаяМатериалы.НомерСтроки
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.АвансовыйОтчет.Прочее КАК АвансовыйОтчетПрочее
	               |		ПО ИсточникДанных.Регистратор = АвансовыйОтчетПрочее.Ссылка
	               |			И ИсточникДанных.НомерСтроки = АвансовыйОтчетПрочее.НомерСтроки
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваровУслуг.Услуги КАК ПоступлениеТоваровУслуг
	               |		ПО ИсточникДанных.Регистратор = ПоступлениеТоваровУслуг.Ссылка
	               |			И ИсточникДанных.НомерСтроки = ПоступлениеТоваровУслуг.НомерСтроки
	               |{ГДЕ
	               |	ИсточникДанных.СуммаОборот КАК СуммаОборот,
	               |	ИсточникДанных.Регистратор.* КАК Регистратор,
	               |	ИсточникДанных.Период КАК Период,
	               ...
	               |	(НАЧАЛОПЕРИОДА(ИсточникДанных.Период, ПОЛУГОДИЕ)) КАК ПериодПолугодие,
	               |	(НАЧАЛОПЕРИОДА(ИсточникДанных.Период, ГОД)) КАК ПериодГод,
	               |	(ЕСТЬNULL(ТребованиеНакладнаяМатериалы.Номенклатура, ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))).* КАК Номенклатура,
	               |	(ЕСТЬNULL(ИсточникДанных.Регистратор.ЭтоИнвестиции, ЛОЖЬ)) КАК ЭтоИнвестиции}
	               |{УПОРЯДОЧИТЬ ПО
	               |	Подразделение.*,
	               |	СтатьяЗатрат.*,
