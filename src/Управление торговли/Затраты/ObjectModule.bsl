...
Процедура УстановитьНачальныеНастройки(ДополнительныеПараметры = Неопределено) Экспорт
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
				   ...
				   |	ВЫБОР КОГДА ИсточникДанных.Регистратор ССЫЛКА Документ.ТребованиеНакладная ТОГДА
	               |		ЕСТЬNULL(ТребованиеНакладнаяМатериалы.Номенклатура, ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)) 
				   |	КОГДА ИсточникДанных.Регистратор ССЫЛКА Документ.АвансовыйОтчет ТОГДА
				   |		ЕСТЬNULL(ВЫРАЗИТЬ(АвансовыйОтчетПрочее.Содержание КАК СТРОКА(100)), """")
				   |	КОГДА ИсточникДанных.Регистратор ССЫЛКА Документ.ПоступлениеТоваровУслуг ТОГДА
