...
Процедура УстановитьНачальныеНастройки(ДополнительныеПараметры = Неопределено) Экспорт
	...
	|		ИЗ
	|			РегистрНакопления.ТоварыКПередачеСоСкладов.ОстаткиИОбороты(&ДатаНач, &ДатаКон, Регистратор, , {(Номенклатура).*, (Склад).*, (ДокументПередачи).* КАК Документ}) КАК ТоварыКПередачеСоСкладовОстаткиИОбороты
	|		
	|		ОБЪЕДИНИТЬ ВСЕ
	|		
	|		ВЫБРАТЬ
	...
	|		ИЗ
	|			РегистрНакопления.ТоварыКПолучениюНаСклады.ОстаткиИОбороты(&ДатаНач, &ДатаКон, Регистратор, , {(Номенклатура).*, (Склад).*, (ДокументПолучения).* КАК Документ}) КАК ТоварыКПолучениюНаСкладыОстаткиИОбороты) КАК ВложенныйЗапрос
	|	
	|	СГРУППИРОВАТЬ ПО
