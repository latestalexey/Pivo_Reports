Перем ТаблицаПрародителей Экспорт;
#Если Клиент Тогда
...
Процедура УстановитьНачальныеНастройки(ДополнительныеПараметры = Неопределено) Экспорт
	Если ТаблицаПрародителей=Неопределено Тогда
		Состояние("Расчет прародителей");
		ТаблицаПрародителей=Прародители(5);
		Состояние("");
	КонецЕсли;
	...
	|	ВложенныйЗапрос.Предок КАК ГруппаВерхнегоУровня,
	...
	|		КОГДА ВложенныйЗапрос.Количество = 0
	|			ТОГДА 0
	|		ИНАЧЕ ВложенныйЗапрос.ВаловаяПрибыль / ВложенныйЗапрос.Количество
	|	КОНЕЦ КАК РентабельностьРуб,
	|	ЕСТЬNULL(ВложенныйЗапрос.ЦенаЗакупа, 0) КАК ЦенаЗакупа,
	|	1 КАК Дельта
	|	//ПОЛЯ_СВОЙСТВА
	|	//ПОЛЯ_КАТЕГОРИИ
	|{ВЫБРАТЬ
	...
	|		СРЕДНЕЕ(ТаблицаЦенЗакупа.ЦенаЗакупа) КАК ЦенаЗакупа,
	|		ВТПрародители.Предок КАК Предок
	|	{ВЫБРАТЬ
	|		Организация,
	|		Проект,
	|		Подразделение,
	...
	|				ГДЕ
	|					ПродажиСебестоимость.Период МЕЖДУ &ДатаНачала И ДОБАВИТЬКДАТЕ(&ДатаКонца, МЕСЯЦ, 1)
	|				{ГДЕ
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|				ЕСТЬNULL(ЗакупкиОбороты.Номенклатура, ЦеныНоменклатурыСрезПоследних.Номенклатура) КАК Номенклатура,
	|				ЕСТЬNULL(ЗакупкиОбороты.ХарактеристикаНоменклатуры, ЦеныНоменклатурыСрезПоследних.ХарактеристикаНоменклатуры) КАК ХарактеристикаНоменклатуры,
	|				ВЫБОР
	|					КОГДА ЕСТЬNULL(ЗакупкиОбороты.КоличествоОборот, 0) = 0
	|							ИЛИ ЕСТЬNULL(ЗакупкиОбороты.СтоимостьОборот, 0) = 0
	|						ТОГДА ЦеныНоменклатурыСрезПоследних.Цена
	|					ИНАЧЕ ЗакупкиОбороты.СтоимостьОборот / ЗакупкиОбороты.КоличествоОборот
	|				КОНЕЦ КАК ЦенаЗакупа
	|			ИЗ
	|				РегистрНакопления.Закупки.Обороты(&ДатаНачала, &ДатаКонца, , {(Номенклатура).*, (ХарактеристикаНоменклатуры).*}) КАК ЗакупкиОбороты
	|					ПОЛНОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(&ДатаКонца, ТипЦен = &ЗакупТранспорт {(Номенклатура).*, (ХарактеристикаНоменклатуры).*}) КАК ЦеныНоменклатурыСрезПоследних
	|					ПО ЗакупкиОбороты.Номенклатура = ЦеныНоменклатурыСрезПоследних.Номенклатура) КАК ТаблицаЦенЗакупа
	|			ПО ВложенныйЗапрос.Номенклатура = ТаблицаЦенЗакупа.Номенклатура
	|				И ВложенныйЗапрос.ХарактеристикаНоменклатуры = ТаблицаЦенЗакупа.ХарактеристикаНоменклатуры
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТПрародители КАК ВТПрародители
	|			ПО ВложенныйЗапрос.Номенклатура = ВТПрародители.Потомок
	|	
	|	СГРУППИРОВАТЬ ПО
	...
	|			ТОГДА СУММА(Себестоимость)
	|		ИНАЧЕ СУММА(Себестоимость) / СУММА(Количество)
	|	КОНЕЦ КАК ЦенаЗТ,
	|	ВЫБОР
	|		КОГДА СУММА(Количество) = 0
	|			ТОГДА СУММА(ВаловаяПрибыль)
	|		ИНАЧЕ СУММА(ВаловаяПрибыль) / СУММА(Количество)
	|	КОНЕЦ КАК РентабельностьРуб,
	|	СРЕДНЕЕ(ЦенаЗакупа),
	|	ВЫБОР
	|		КОГДА СУММА(Количество) = 0
	|			ТОГДА СУММА(Себестоимость)
	|		ИНАЧЕ СУММА(Себестоимость) / СУММА(Количество)
	|	КОНЕЦ - СРЕДНЕЕ(ЕСТЬNULL(ЦенаЗакупа, 0)) КАК Дельта
	|	//ИТОГИ_СВОЙСТВА
	|	//ИТОГИ_КАТЕГОРИИ
	|ПО
	|	ОБЩИЕ
	...
	|	//ПСЕВДОНИМЫ_КАТЕГОРИИ
	|}";
	УниверсальныйОтчет.ПостроительОтчета.Параметры.Вставить("ЗакупТранспорт",Справочники.ИТИКонстанты.ТипЦен.Указатель);
	...
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить("ГруппаВерхнегоУровня", "Гр. верх. уровня");
	...
	УниверсальныйОтчет.ДобавитьПоказатель("ЦенаЗТ", "Цена З+Т по валовой прибыли", Истина, "ЧЦ=15; ЧДЦ=2", "Цены","Цены");
	УниверсальныйОтчет.ДобавитьПоказатель("ЦенаЗакупа", "Цена З+Т по закупу", Истина, "ЧЦ=15; ЧДЦ=2","Цены");
	УниверсальныйОтчет.ДобавитьПоказатель("Дельта", "Разница", Истина, "ЧЦ=15; ЧДЦ=2","Цены");
	...
	//KAV++ Крапивин Андрей. 19.02.2015 17:36:46
	УниверсальныйОтчет.ДобавитьПоказатель("РентабельностьРуб", "Рентабельность, руб", Истина, "ЧЦ=15; ЧДЦ=2");
...
//KAV МАГИЯ!!!!! Получаем таблицу номерклатуры с ее родителем верхнего уровня
...
Функция Прародители(МаксимальнаяДлинаПути) Экспорт
    ...
    Эпилог = "ВЫБРАТЬ НачалоДуги Предок, КонецДуги Потомок ПОМЕСТИТЬ ВТПрародители ИЗ ЗамыканияДлины#2
			
            | ГДЕ НачалоДуги <> КонецДуги И НачалоДуги.Родитель = Значение(Справочник.Номенклатура.ПустаяСсылка) ИНДЕКСИРОВАТЬ ПО Потомок;";
