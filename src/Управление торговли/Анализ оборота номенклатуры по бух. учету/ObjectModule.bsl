...
Процедура ДоработатьКомпоновщикПередВыводом() Экспорт
	...
	ПараметрКонецПериода = КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("КонецПериода"));
		...
		Параметр = КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("СкладДляАнализа"));
	Если Параметр <> Неопределено Тогда
		Параметр.Значение = Справочники.КатегорииОбъектов.НайтиПоКоду("000000138");
		Параметр.Использование = Истина;
	КонецЕсли;
	Параметр = КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("СкладВПути"));
	Если Параметр <> Неопределено Тогда
		Параметр.Значение = Справочники.Склады.НайтиПоКоду("000000043");
		Параметр.Использование = Истина;
	КонецЕсли;
	Параметр = КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("ТипЦенЗакуп"));
	Если Параметр <> Неопределено Тогда
		Параметр.Значение = Справочники.ИТИКонстанты.ТЦ_Закупочная.Указатель;
		Параметр.Использование = Истина;
	КонецЕсли;
	
	Параметр1 = КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("КварталОтчета"));
	Параметр2 = КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("ВводОстатковКв"));
	ДатаОтчета= КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("ПериодАнализа"));
	Если ДатаОтчета<>Неопределено Тогда
		Если ЗначениеЗаполнено(ДатаОтчета.Значение) Тогда
			ДатаОтчета=НачалоДня(ДатаОтчета.Значение);
		Иначе
	...
	Если Параметр1 <> Неопределено И Параметр2 <> Неопределено Тогда
		...
		|		ПО КорректировкаЗаписейРегистров.Ссылка = КатегорииОбъектов.Объект
		|			И (КатегорииОбъектов.Категория = &ИнициализацияКвартала)
		|ГДЕ
		|	НЕ КорректировкаЗаписейРегистров.ПометкаУдаления
		|	И КорректировкаЗаписейРегистров.Дата <= &НачалоКвартала
		|
		|УПОРЯДОЧИТЬ ПО
		|	Дата УБЫВ";
		
		Запрос.УстановитьПараметр("ИнициализацияКвартала", Справочники.КатегорииОбъектов.НайтиПоНаименованию("Инициализация квартала"));
		Запрос.УстановитьПараметр("НачалоКвартала", ДатаОтчета);
		...
		Если ВыборкаДетальныеЗаписи.Следующий() Тогда
			Параметр1.Значение=ВыборкаДетальныеЗаписи.Дата;
			Параметр1.Использование=Истина;
			
			Параметр2.Значение=ВыборкаДетальныеЗаписи.Ссылка;
			Параметр2.Использование=Истина;
		Иначе
			Параметр1.Значение=ДатаОтчета;
			Параметр1.Использование=Истина;
			
			Параметр2.Значение=Документы.КорректировкаЗаписейРегистров.ПустаяСсылка();
			Параметр2.Использование=Истина;
		КонецЕсли;
	КонецЕсли;
