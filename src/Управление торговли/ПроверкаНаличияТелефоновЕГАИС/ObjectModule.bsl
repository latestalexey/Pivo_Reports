...
Процедура УстановитьНачальныеНастройки(ДополнительныеПараметры = Неопределено) Экспорт
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	КонтактнаяИнформацияГрузополучатель.Объект ЕСТЬ НЕ NULL  КАК ЕстьТелефонГрузополучатель,
	               |	КонтактнаяИнформацияКонтрагент.Объект ЕСТЬ НЕ NULL  КАК ЕстьТелефонКонтрагент,
	               |	РеализацияТоваровУслуг.Контрагент,
	               |	РеализацияТоваровУслуг.Грузополучатель
	               |{ВЫБРАТЬ
	               |	Накладная.*,
	               |	ЕстьТелефонГрузополучатель,
	               |	ЕстьТелефонКонтрагент,
	               |	Контрагент.*,
	               |	Грузополучатель.*}
	               |ИЗ
	               ...
	               |			И (КонтактнаяИнформацияКонтрагент.Вид = &ТелефонЕГАИС)
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КонтактнаяИнформация КАК КонтактнаяИнформацияГрузополучатель
	               |		ПО РеализацияТоваровУслуг.Грузополучатель = КонтактнаяИнформацияГрузополучатель.Объект
	               |			И (КонтактнаяИнформацияГрузополучатель.Вид = &ТелефонЕГАИС)
	               |ГДЕ
	               ...
	               |	РеализацияТоваровУслуг.Грузополучатель.*}
	               |ИТОГИ
	               |	МАКСИМУМ(ЕстьТелефонГрузополучатель),
	               |	МАКСИМУМ(ЕстьТелефонКонтрагент)
	               |ПО
	               |	ОБЩИЕ
	               |{ИТОГИ ПО
	               |	Накладная.*,
	...
	УниверсальныйОтчет.ПостроительОтчета.Параметры.Вставить("ТелефонЕГАИС",Справочники.ВидыКонтактнойИнформации.НайтиПоНаименованию("Телефон ответственного за ЕГАИС").Ссылка);
	УниверсальныйОтчет.ПостроительОтчета.Параметры.Вставить("Организация",Справочники.ИТИКонстанты.ОргГарц.Указатель);
	...
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить( "ЕстьТелефонКонтрагент",				"Есть телефон у контрагента");
	УниверсальныйОтчет.мСтруктураПредставлениеПолей.Вставить( "ЕстьТелефонГрузополучатель",			"Есть телефон у грузополучателя");
	...
	//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!Пример
	УниверсальныйОтчет.ДобавитьПоказатель("ЕстьТелефонКонтрагент",		"Контрагент",		Истина, "БЛ=Нет; БИ=Да","ЕстьТелефон","Есть телефон");
	УниверсальныйОтчет.ДобавитьПоказатель("ЕстьТелефонГрузополучатель", "Грузополучатель",	Истина, "БЛ=Нет; БИ=Да","ЕстьТелефон","Есть телефон");
