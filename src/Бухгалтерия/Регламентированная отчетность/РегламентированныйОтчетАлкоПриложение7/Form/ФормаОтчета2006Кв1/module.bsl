...
Функция СобратьДанныеТекущегоТаблПоля(ТабличноеПоле) Экспорт
	...
	Для Инд = 0 По ТабличноеПоле.Области.Количество() - 1 Цикл
		...
		Если ТабличноеПолеИмя = "ПолеТабличногоДокументаДекларация"
		   И Найти(ИмяПоказателя, "П000010001008") > 0 Тогда
			НомерСтроки = Сред(ИмяПоказателя, 15);
			ЗначениеПоказателя = СтруктураГруппы1[Число(НомерСтроки) - 1].П000010001008;
		КонецЕсли;
...
Процедура ОбновитьСтруктуруМногострочногоРаздела(ИмяГруппы) Экспорт
	...
	Для НСтр = 1 По СтруктураГруппы.Количество() Цикл
		...
		Для Каждого Колонка Из СтруктураГруппы.Колонки Цикл
			...
			Если ИмяКолонки = "П000010001008" Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ЗаполнитьСведенияОбОрганизации(ВПрограммеИзмененаДатаПодписи = Ложь)
	...
	Если (НЕ ВПрограммеИзмененаДатаПодписи) И (Результат <> Неопределено) Тогда
		...
		Иначе
			...
			мАдрЮРТитульныйВыгрузка = Результат.АдрЮР;
		КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из СтруктураГруппы Цикл
		Для Каждого КолонкаТаблицы Из СтруктураГруппы.Колонки Цикл
			...
			Если ОбластьЯчейкиТаблицы.СодержитЗначение = Истина Тогда
				...
				Если ИмяГруппы = мИдГруппы1 И КолонкаТаблицы.Имя = "П000010001008" Тогда
					ЗначениеЯчейки = РегламентированнаяОтчетность.ПредставлениеАдресаВФормате9Запятых(ЗначениеЯчейки);
				КонецЕсли;

				ОбластьЯчейкиТаблицы.Значение = ЗначениеЯчейки;

				// Обеспечим маску ввода ИННа 12 символов.
				Если Найти(ОбластьЯчейкиТаблицы.Имя, "П000010001006") > 0 Тогда
					ОбластьЯчейкиТаблицы.ЭлементУправления.Маска = "999999999999";
				КонецЕсли;

				// Обеспечим маску ввода КПП 9 символов.
				Если Найти(ОбластьЯчейкиТаблицы.Имя, "П000010001007") > 0 Тогда
...
Процедура ОчиститьТабличноеПоле( ВыбТабличноеПоле ) Экспорт
	...
	Для Инд = 0 По ВыбТабличноеПоле.Области.Количество() - 1 Цикл
		...
		Если ИмяГруппыДопСтрок = мИдГруппы1 Тогда
			Если Найти(ТекущаяОбласть.Имя, "П000010001008") > 0 Тогда
				...
				СтруктураГруппы[Число(НомерСтроки) - 1].П000010001008 = "";
			КонецЕсли;
		КонецЕсли;
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если СписокСохранения.Свойство( "ДанныеМногострочныхРазделов", ДанныеМногострочныхРазделов) Тогда
		...
		Если мСтруктураМногострочныхРазделов.П0000100010.Колонки.Найти("П000010001409") = Неопределено Тогда
			мСтруктураМногострочныхРазделов.П0000100010.Колонки.Добавить("П000010001409", ОбщегоНазначения.ПолучитьОписаниеТиповДаты(ЧастиДаты.Дата));
		КонецЕсли;
	...
	Для Каждого ПоказателиСтраницы Из ПоказателиОтчета Цикл
		...
		Для Каждого Показатель Из ПоказателиТаблПоле Цикл
			...
			Если Найти(ИмяПоказателя, "П000010001008") > 0 Тогда
				Продолжить;
			КонецЕсли;
	...
	мТитульный.Области.АдрЮР.Значение = РегламентированнаяОтчетность.ПредставлениеАдресаВФормате9Запятых(мАдрЮРТитульныйВыгрузка);
...
Процедура Расчет(ИмяТекущейСтраницыПанели = "", КолонкаИзменений = Неопределено, НомерИзменяемойСтроки = Неопределено) Экспорт
	...
	// Процедура оставлена для совместимости. Задействована в других механизмах.
			...
			Для НСтр = 1 По  ИсходноеКоличествоСтрокГруппы Цикл
				...
				Для Каждого Колонка Из СтруктураГруппы.Колонки Цикл
					...
					Если ОбластьЯчейки.СодержитЗначение = Истина Тогда
						...
						// Обеспечим маску ввода ИННа 12 символов.
						Если Найти(ОбластьЯчейки.Имя, "П000010001006") > 0 Тогда
							ОбластьЯчейки.ЭлементУправления.Маска = "999999999999";
						КонецЕсли;

						// Обеспечим маску ввода КПП 9 символов.
						Если Найти(ОбластьЯчейки.Имя, "П000010001007") > 0 Тогда
...
Процедура ПолеТабличногоДокументаДекларацияВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если ((Найти(Область.Имя, "П000010001003") > 0)
	 Или (Найти(Область.Имя, "П000010001002") > 0))
	 И (НЕ мАвтоВыборКодов) Тогда

		СтандартнаяОбработка = Ложь;
		НСтр = Сред(Область.Имя,15);
		ТекКод = мДекларация.Области["П000010001003_" + НСтр].Значение;
		...
		Если Результат <> Неопределено Тогда
			...
			Если РегламентированнаяОтчетность.ПустоеЗначение(Результат[0]) Тогда
				мДекларация.Области["П000010001002_" + НСтр].Значение = "";
				мДекларация.Области["П000010001003_" + НСтр].Значение = "";

			Иначе
				мДекларация.Области["П000010001002_" + НСтр].Значение = Результат[1];
				мДекларация.Области["П000010001003_" + НСтр].Значение = Результат[0];
			КонецЕсли;

			Модифицированность = Истина;

		КонецЕсли;
	ИначеЕсли (Найти(Область.Имя, "П000010001008") > 0) И (НЕ мАвтоВыборКодов) Тогда
		СтандартнаяОбработка = Ложь;
		...
		СтруктураГруппы1 = Неопределено;
		мСтруктураМногострочныхРазделов.Свойство(мИдГруппы1, СтруктураГруппы1);
		Адр9Зпт = СтруктураГруппы1[Число(НомерСтроки) - 1].П000010001008;
		...
		Если ВводАдреса.ОткрытьМодально() Тогда
			...
			ПолученныйАдрес = ВводАдреса.НачальноеЗначениеВыбора.Представление;
			Если ПолученныйАдрес = ",,,,,,,,," Тогда
				ПолученныйАдрес = "";
			КонецЕсли;

			// в табл. запишем адрес для выгрузки, а на лист, адрес для пользователя.
			СтруктураГруппы1[Число(НомерСтроки) - 1].П000010001008 = ПолученныйАдрес;

			// Запишем на лист адрес для пользователя.
			Область.Значение = РегламентированнаяОтчетность.ПредставлениеАдресаВФормате9Запятых(ПолученныйАдрес);
			Модифицированность = Истина;
		КонецЕсли;
...
Процедура ПолеТабличногоДокументаТитульныйВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если (НЕ мАвтоВыборКодов) И (Область.Имя = "АдрЮР") Тогда
		...
		Если ВводАдреса.ОткрытьМодально() Тогда
			...
			мАдрЮРТитульныйВыгрузка = СокрЛП(ВводАдреса.НачальноеЗначениеВыбора.Представление);
			Если мАдрЮРТитульныйВыгрузка = ",,,,,,,,," Тогда
				мАдрЮРТитульныйВыгрузка = "";
			КонецЕсли;

			Область.Значение = РегламентированнаяОтчетность.ПредставлениеАдресаВФормате9Запятых(мАдрЮРТитульныйВыгрузка);
...
Функция ПроверитьПоказатели() Экспорт
	...
	РегламентированнаяОтчетность.роСообщитьВТаблицуСообщений(ЭтаФорма, "Отчетом не предусмотрена возможность выгрузки!", "Титульный", "");
	Возврат Ложь;
	
КонецФункции

Процедура УниверсальнаяВыгрузка() Экспорт
...
СтруктураГруппы1.Колонки.Добавить("П000010001001",ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(6)); // номер
СтруктураГруппы1.Колонки.Добавить("П000010001002", ОписаниеТиповСтрока254); // наименование продукции
СтруктураГруппы1.Колонки.Добавить("П000010001003",ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(3)); // код
СтруктураГруппы1.Колонки.Добавить("П000010001004", ОписаниеТиповСтрока254); // Получатель
СтруктураГруппы1.Колонки.Добавить("П000010001005",ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(2)); // Код региона РФ
СтруктураГруппы1.Колонки.Добавить("П000010001006",ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(12)); // ИНН
СтруктураГруппы1.Колонки.Добавить("П000010001007",ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(9)); // КПП
СтруктураГруппы1.Колонки.Добавить("П000010001008", ОписаниеТиповСтрока254); // Адрес
СтруктураГруппы1.Колонки.Добавить("П000010001109",ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(15)); // Серия
СтруктураГруппы1.Колонки.Добавить("П000010001209", ОписаниеТиповСтрока254); // номер
СтруктураГруппы1.Колонки.Добавить("П000010001309", ОписаниеТиповДата); // дата
СтруктураГруппы1.Колонки.Добавить("П000010001409", ОписаниеТиповДата); // дата
СтруктураГруппы1.Колонки.Добавить("П000010001010", ОписаниеТиповСтрока254); // Кем выд.
СтруктураГруппы1.Колонки.Добавить("П000010001111", ОписаниеТиповСтрока254); // Номер уведомления
СтруктураГруппы1.Колонки.Добавить("П000010001211", ОписаниеТиповДата); // Дата
СтруктураГруппы1.Колонки.Добавить("П000010001012", ОписаниеТиповДата); // Дата отгрузки
СтруктураГруппы1.Колонки.Добавить("П000010001013", ОписаниеТиповСтрока254); // номер ТТС
СтруктураГруппы1.Колонки.Добавить("П000010001014", ОписаниеТиповСтрока254); // номер груз. там. декл.
СтруктураГруппы1.Колонки.Добавить("П000010001015", ОписаниеТиповДата); // номер груз. там. декл.
СтруктураГруппы1.Колонки.Добавить("П000010001016", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14, 4));
