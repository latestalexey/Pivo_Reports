...
Процедура ЗаполнитьАвтоБухгалтерия(СписокОрганизаций) Экспорт
	...
	Запрос.Текст =
#Область Запрос_оборотов //14.10.2016 10:32:03
	"ВЫБРАТЬ
	...
	|	ВЫРАЗИТЬ(СведенияОбОборотах.Грузоотправитель.НаименованиеПолное КАК СТРОКА(200)) КАК П000010000106,
	|	ВЫРАЗИТЬ(ЕСТЬNULL(КонтактнаяИнформация.Представление, """") КАК СТРОКА(100)) КАК П000010000107,
	|	СведенияОбОборотах.Грузоотправитель.ИНН КАК П000010000108,
	|	СведенияОбОборотах.Грузоотправитель.КПП КАК П000010000109,
	...
	|	"""" КАК П000010000116,
	|	ВЫБОР
	|		КОГДА СведенияОбОборотах.Регистратор.ДатаВходящегоДокумента = &ПустаяДата
	|			ТОГДА СведенияОбОборотах.Регистратор.Дата
	|		ИНАЧЕ СведенияОбОборотах.Регистратор.ДатаВходящегоДокумента
	|	КОНЕЦ КАК П000010000117,
	|	ВЫБОР
	|		КОГДА СведенияОбОборотах.Регистратор.НомерВходящегоДокумента = &ПустаяСтрока
	|			ТОГДА СведенияОбОборотах.Регистратор.Номер
	|		ИНАЧЕ СведенияОбОборотах.Регистратор.НомерВходящегоДокумента
	|	КОНЕЦ КАК П000010000118,
	|	"""" КАК П000010000119,
	|	СУММА(ВЫБОР
	|			КОГДА СведенияОбОборотах.Регистратор ССЫЛКА Документ.ВозвратТоваровОтПокупателя
	|				ТОГДА 0
	|			ИНАЧЕ СведенияОбОборотах.КоличествоОборот * СведенияОбАлкогольнойПродукции.КоэффПересчетаДал
	|		КОНЕЦ) КАК П000010000120,
	|	СведенияОбОборотах.Контрагент КАК Контрагент,
	|	СведенияОбОборотах.Организация,
	|	СведенияОбОборотах.Подразделение
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|		ХозрасчетныйОбороты.КоличествоОборотДт КАК КоличествоОборот,
	|		ХозрасчетныйОбороты.Регистратор.Контрагент КАК Контрагент,
	|		ВЫБОР
	|			КОГДА ХозрасчетныйОбороты.Регистратор.Грузоотправитель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|				ТОГДА ХозрасчетныйОбороты.Регистратор.Контрагент
	|			ИНАЧЕ ХозрасчетныйОбороты.Регистратор.Грузоотправитель
	|		КОНЕЦ КАК Грузоотправитель
	|	ИЗ
	|		РегистрБухгалтерии.Хозрасчетный.Обороты(&ДатаНач, &ДатаКон, Регистратор, Счет В ИЕРАРХИИ (&Счет), &ВидСубконто, Организация В (&СписокОрганизаций), КорСчет В ИЕРАРХИИ (&КорСчет), ) КАК ХозрасчетныйОбороты
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|	ГДЕ
	|		ХозрасчетныйОбороты.Регистратор.Контрагент <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|		И ХозрасчетныйОбороты.Регистратор.ЭтоВозврат
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|	ГДЕ
	|		ХозрасчетныйОбороты.КоличествоОборотДт > 0) КАК СведенияОбОборотах
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КонтактнаяИнформация КАК КонтактнаяИнформация
	|		ПО (ВЫБОР
	|				КОГДА СведенияОбОборотах.Грузоотправитель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|					ТОГДА СведенияОбОборотах.Контрагент
	|				ИНАЧЕ СведенияОбОборотах.Грузоотправитель
	...
	|		ПО СведенияОбОборотах.Регистратор.Контрагент = ЛицензииПоставщиковАлкогольнойПродукцииСрезПоследних.Поставщик
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СведенияОбАлкогольнойПродукции КАК СведенияОбАлкогольнойПродукции
	|		ПО СведенияОбОборотах.Номенклатура = СведенияОбАлкогольнойПродукции.Номенклатура
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	СведенияОбОборотах.Организация,
	|	СведенияОбОборотах.Подразделение,
	|	СведенияОбОборотах.Грузоотправитель.ИНН,
	|	СведенияОбОборотах.Грузоотправитель.КПП,
	|	СведенияОбОборотах.Контрагент,
	|	ВЫРАЗИТЬ(СведенияОбОборотах.Грузоотправитель.НаименованиеПолное КАК СТРОКА(200)),
	|	ВЫБОР
	|		КОГДА СведенияОбОборотах.Регистратор.ДатаВходящегоДокумента = &ПустаяДата
	|			ТОГДА СведенияОбОборотах.Регистратор.Дата
	|		ИНАЧЕ СведенияОбОборотах.Регистратор.ДатаВходящегоДокумента
...
#КонецОбласти	
	...
	Запрос.УстановитьПараметр("ПустаяСтрока",	"");
	Запрос.Текст =
#Область Запрос_возвратов //14.10.2016 10:39:10
	"ВЫБРАТЬ
	|	ВложенныйЗапрос.П000020000101,
	|	ВложенныйЗапрос.П000020000102,
	|	ВложенныйЗапрос.П000020000103,
	|	ВложенныйЗапрос.П000020000104,
	|	ВложенныйЗапрос.П000020000105,
	|	ВложенныйЗапрос.П000020000106,
	|	ВложенныйЗапрос.П000020000107,
	|	ВложенныйЗапрос.П000020000108,
	|	ВложенныйЗапрос.П000020000109,
	|	ВложенныйЗапрос.П000020000110,
	|	ВложенныйЗапрос.П000020000111,
	|	ВложенныйЗапрос.П000020000112,
	|	ВложенныйЗапрос.П000020000113,
	|	ВложенныйЗапрос.П000020000114,
	|	ВложенныйЗапрос.П000020000115,
	|	ВложенныйЗапрос.П000020000116,
	|	ВложенныйЗапрос.П000020000117,
	|	ВложенныйЗапрос.П000020000118,
	|	ВложенныйЗапрос.П000020000119,
	|	СУММА(ВложенныйЗапрос.П000020000120) КАК П000020000120,
	...
	|		ВЫРАЗИТЬ(ОборотыВозвраты.ПроизводительИмпортер.НаименованиеПолное КАК СТРОКА(200)) КАК П000020000103,
	|		ОборотыВозвраты.ПроизводительИмпортер.ИНН КАК П000020000104,
	|		ОборотыВозвраты.ПроизводительИмпортер.КПП КАК П000020000105,
	|		ВЫРАЗИТЬ(ОборотыВозвраты.Регистратор.Контрагент.НаименованиеПолное КАК СТРОКА(200)) КАК П000020000106,
	|		ВЫРАЗИТЬ(ЕСТЬNULL(КонтактнаяИнформация.Представление, """") КАК СТРОКА(100)) КАК П000020000107,
	|		ОборотыВозвраты.Контрагент.ИНН КАК П000020000108,
	|		ОборотыВозвраты.Контрагент.КПП КАК П000020000109,
	...
	|		"""" КАК П000020000115,
	|		"""" КАК П000020000116,
	|		ОборотыВозвраты.ВхДата КАК П000020000117,
	|		ОборотыВозвраты.ВхНомер КАК П000020000118,
	|		"""" КАК П000020000119,
	|		(ЕСТЬNULL(ОборотыВозвраты.КоличествоОборотДт, 0) - ЕСТЬNULL(ОборотыВозвраты.КоличествоОборотКт, 0)) * СведенияОбАлкогольнойПродукции.КоэффПересчетаДал КАК П000020000120,
	|		ОборотыВозвраты.Контрагент КАК Контрагент,
	|		ОборотыВозвраты.Организация КАК Организация,
	|		ОборотыВозвраты.Подразделение КАК Подразделение
	|	ИЗ
	|		(ВЫБРАТЬ
	|			ХозрасчетныйОбороты.Регистратор КАК Регистратор,
	|			ХозрасчетныйОбороты.Субконто1 КАК Номенклатура,
	...
	|			КОНЕЦ КАК ВхДата
	|		ИЗ
	|			РегистрБухгалтерии.Хозрасчетный.Обороты(&ДатаНач, &ДатаКон, Регистратор, Счет В ИЕРАРХИИ (&Счет), &ВидСубконто, Организация В (&СписокОрганизаций), , ) КАК ХозрасчетныйОбороты) КАК ОборотыВозвраты
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СведенияОбАлкогольнойПродукции КАК СведенияОбАлкогольнойПродукции
	|			ПО ОборотыВозвраты.Номенклатура = СведенияОбАлкогольнойПродукции.Номенклатура
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КонтактнаяИнформация КАК КонтактнаяИнформация
	|			ПО (КонтактнаяИнформация.Вид = &ЮрАдрес)
	|				И ОборотыВозвраты.Контрагент = КонтактнаяИнформация.Объект
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЛицензииПоставщиковАлкогольнойПродукции.СрезПоследних(&ДатаКон, ) КАК ЛицензииПоставщиковАлкогольнойПродукцииСрезПоследних
	|			ПО ОборотыВозвраты.Контрагент = ЛицензииПоставщиковАлкогольнойПродукцииСрезПоследних.Поставщик
	|	ГДЕ
	|		ОборотыВозвраты.Регистратор ССЫЛКА Документ.ВозвратТоваровОтПокупателя) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.П000020000101,
	|	ВложенныйЗапрос.П000020000102,
