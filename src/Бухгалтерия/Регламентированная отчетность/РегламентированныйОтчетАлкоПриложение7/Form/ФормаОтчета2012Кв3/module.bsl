...
Процедура ЗаполнитьАвтоБухгалтерия(СписокОрганизаций) Экспорт
	...
	|	ВЫБОР
	|		КОГДА СведенияОбОборотах.Регистратор.Грузоотправитель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|			ТОГДА ВЫРАЗИТЬ(СведенияОбОборотах.Регистратор.Контрагент.НаименованиеПолное КАК СТРОКА(200))
	|		ИНАЧЕ ВЫРАЗИТЬ(СведенияОбОборотах.Регистратор.Грузоотправитель.НаименованиеПолное КАК СТРОКА(200))
	|	КОНЕЦ КАК П000010000106,
	|	ВЫРАЗИТЬ(ЕСТЬNULL(КонтактнаяИнформация.Представление, """") КАК СТРОКА(100)) КАК П000010000107,
	|	ВЫБОР
	|		КОГДА СведенияОбОборотах.Регистратор.Грузоотправитель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|			ТОГДА СведенияОбОборотах.Регистратор.Контрагент.ИНН
	|		ИНАЧЕ СведенияОбОборотах.Регистратор.Грузоотправитель.ИНН
	|	КОНЕЦ КАК П000010000108,
	|	ВЫБОР
	|		КОГДА СведенияОбОборотах.Регистратор.Грузоотправитель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|			ТОГДА СведенияОбОборотах.Регистратор.Контрагент.КПП
	|		ИНАЧЕ СведенияОбОборотах.Регистратор.Грузоотправитель.КПП
	...
	|	ВЫБОР
	|		КОГДА СведенияОбОборотах.Регистратор.Грузоотправитель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|			ТОГДА СведенияОбОборотах.Регистратор.Контрагент
	|		ИНАЧЕ СведенияОбОборотах.Регистратор.Грузоотправитель
	...
	|	ИЗ
	|		РегистрБухгалтерии.Хозрасчетный.Обороты(&ДатаНач, &ДатаКон, Регистратор, Счет В ИЕРАРХИИ (&Счет), &ВидСубконто, Организация В (&СписокОрганизаций), КорСчет В ИЕРАРХИИ (&КорСчет), ) КАК ХозрасчетныйОбороты) КАК СведенияОбОборотах
	...
	|		ПО (СведенияОбАлкогольнойПродукции.Номенклатура = СведенияОбОборотах.Субконто1)
	|
	|СГРУППИРОВАТЬ ПО
