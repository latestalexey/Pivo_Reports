...
Процедура ЗаполнитьАвтоБухгалтерия(СписокОрганизаций) Экспорт
	...
	|	"""" КАК П000010000115,
	|	"""" КАК П000010000116,
	|	СведенияОбОборотах.Регистратор.Дата КАК П000010000117,
	|	ВЫБОР
	...
	|		ХозрасчетныйОбороты.Регистратор.Контрагент
	|	ИЗ
	|		РегистрБухгалтерии.Хозрасчетный.Обороты(&ДатаНач, &ДатаКон, Регистратор, Счет В ИЕРАРХИИ (&Счет), &ВидСубконто, Организация В (&СписокОрганизаций), КорСчет В ИЕРАРХИИ (&Счет), ) КАК ХозрасчетныйОбороты
	|	ГДЕ
	...
	|	СведенияОбОборотах.Регистратор.Дата,
	|	СведенияОбОборотах.Организация,
	|	СведенияОбОборотах.Подразделение,
