...
Процедура ЗаполнитьАвто()
	...
	ТаблицаРаздела.Свернуть("П000000000006, П000000000007", "П000000000016");
	...
	Для Каждого Стр Из ТаблицаРаздела Цикл
		...
		Если НЕ ЗначениеЗаполнено(Стр.П000000000006) И НЕ ЗначениеЗаполнено(Стр.П000000000007) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ПолеТабличногоДокументаПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если НЕ ((Страница = "Титульный") Или (Страница = "ФизЛица")) Тогда
		...
		Если Найти(ОбластьИмя, "П000000000016_") > 0 Тогда
			...
			Расчет("");
		ИначеЕсли (Найти(ОбластьИмя, "П000000000008") > 0
		       Или Найти(ОбластьИмя, "П000010003008") > 0) И (мАвтоВыборКодов) Тогда
...
СтруктураГруппы1.Колонки.Добавить("П000000000006",ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(10)); // ИНН
...
СтруктураГруппы2.Колонки.Добавить("П000010003006",ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(10)); // ИНН
