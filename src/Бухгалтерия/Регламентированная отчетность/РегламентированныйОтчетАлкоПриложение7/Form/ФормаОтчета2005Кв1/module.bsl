...
Функция ПолучитьТаблицуВариантовЗаполнения(ИмяСтраницыПанели, ПолучитьЭталонную = Ложь)
	...
	Если Т = Неопределено Тогда
		...
		Т.Колонки.Добавить( "КодПоказателя",     ОписаниеТиповСтрока50  );
		Т.Колонки.Добавить( "ВариантЗаполнения", ОписаниеТиповЧисло1    );
		Т.Колонки.Добавить( "ЗначениеАвто",      ОписаниеТиповЧисло15   );
		Т.Колонки.Добавить( "Дельта",            ОписаниеТиповЧисло15   );
		Т.Колонки.Добавить( "Комментарий",       ОписаниеТиповСтрока100 );
		Т.Колонки.Добавить( "КодПоказателяПоСтруктуре",       ОписаниеТиповСтрока50 );
...
Процедура ЗаполнитьСведенияОбОрганизации()
	...
	Если (РегламентированнаяОтчетность.ПустоеЗначение(мРаздел1.Области.ОргКодОКАТО.Значение)) ИЛИ (мВПрограммеИзмененаОрганизация) Тогда

		мРаздел1.Области.ОргКодОКАТО.Значение = КодОКАТО;
	...
	Если мВПрограммеИзмененаОрганизация Тогда
		...
		Модифицированность = ?(СокрЛП(мРаздел1.Области.ОргДиректор.Значение) = СокрЛП(ФИОРук), Ложь, Истина);
		мРаздел1.Области.ОргДиректор.Значение  = ФИОРук;

		Модифицированность = ?(мРаздел1.Области.ОргБухгалтер.Значение = ФИОБух, Ложь, Истина);
		мРаздел1.Области.ОргБухгалтер.Значение = ФИОБух;
...
Процедура ОчиститьТабличноеПоле( ВыбТабличноеПоле ) Экспорт
	...
	СписокПоказателейНеПодлежащихОчистке.Добавить("АттСерия");
	СписокПоказателейНеПодлежащихОчистке.Добавить("АттНомер");
	СписокПоказателейНеПодлежащихОчистке.Добавить("АттДата");
	СписокПоказателейНеПодлежащихОчистке.Добавить("АттВыдан");
...
Процедура Расчет(ИмяТекущейСтраницыПанели = "") Экспорт
	...
	// строка 020, итог по графе 13
	//
	мРаздел1.Области.П000010002013.Значение = РасчетСуммМногострочнойЧасти("П000010001013");
...
//   число.
//
Функция РасчетСуммМногострочнойЧасти(Парам) 
...
Процедура ПолеТабличногоДокументаФормаОтчетаПриВыборе(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли (НЕ ЭлементыФормы.ОтклВыборЗначений.Значение) И ((Область.Имя = "ОргЮрАдрес") Или ( Область.Имя = "ОргАдресПроизводства") Или (Найти(Область.Имя, "П000010001006") > 0)) Тогда

	//ИначеЕсли (Область.Имя = "ОргЮрАдрес") Или ( Область.Имя = "ОргАдресПроизводства") Тогда
	...
	Для Каждого Страница Из мСтруктураМногостраничныхРазделов.Раздел1 Цикл
		...
		Если НомерСтраницы = 1 Тогда
			...
			Если РегламентированнаяОтчетность.ПустоеЗначение(Страница.Данные.ОргАдресПроизводства) Тогда
				...
				СтатусВозврата = Ложь;
			КонецЕсли;
			Если РегламентированнаяОтчетность.ПустоеЗначение(Страница.Данные.ОргКодОКАТО) Тогда
				РегламентированнаяОтчетность.роСообщитьВТаблицуСообщений(ЭтаФорма, "На странице 1 не указан код территории!", "Раздел1", "ОргКодОКАТО", 1);
				СтатусВозврата = Ложь;
			КонецЕсли;
		КонецЕсли;
...
Функция ПолучитьПоказатель(ТекДанные, Показатель, Инд)
	...
	Если Показатель.КодПоказателяПоСоставу = "П000010001005" Тогда
		Возврат ОтформатироватьПоказатель(ТекДанные["П000010001105_" + Инд], "С")
					+ "," + ОтформатироватьПоказатель(ТекДанные["П000010001205_" + Инд], "С");
	ИначеЕсли Показатель.КодПоказателяПоСоставу = "П000010001007" Тогда
		Возврат ОтформатироватьПоказатель(ТекДанные["П000010001107_" + Инд], "С")
					+ "," + ОтформатироватьПоказатель(ТекДанные["П000010001207_" + Инд], "С")
					+ "," + ОтформатироватьПоказатель(ТекДанные["П000010001307_" + Инд], "Д");
	ИначеЕсли Показатель.КодПоказателяПоСоставу = "П000010001009" Тогда
		Возврат ОтформатироватьПоказатель(ТекДанные["П000010001109_" + Инд], "С")
					+ "," + ОтформатироватьПоказатель(ТекДанные["П000010001209_" + Инд], "Д")
					+ "," + ОтформатироватьПоказатель(ТекДанные["П000010001309_" + Инд], "Ч");
	ИначеЕсли Показатель.КодПоказателяПоСоставу = "П000010001010" Тогда		
		Возврат ОтформатироватьПоказатель(ТекДанные["П000010001110_" + Инд], "Д")
					+ "," + ОтформатироватьПоказатель(ТекДанные["П000010001210_" + Инд], "Ч");
	Иначе
...
Функция УниверсальнаяВыгрузка(Текст = Неопределено, ПроверятьБлокировки = Истина) Экспорт
	...
	Для Каждого Страница Из мСтруктураМногостраничныхРазделов.Раздел1 Цикл
		...
		Если ПервыйПроход Тогда // если это первый проход, то выгрузим сведения об организации
			...
			Текст.ДобавитьСтроку("ОКАТО:" + ВРЕГ(СокрЛП(Страница.Данные.ОргКодОКАТО)));
	...
	Если РегламентированнаяОтчетность.ПустоеЗначение(мСтруктураМногостраничныхРазделов.Раздел1[0].Данные.АттСерия) ИЛИ
		РегламентированнаяОтчетность.ПустоеЗначение(мСтруктураМногостраничныхРазделов.Раздел1[0].Данные.АттНомер) Тогда
		
		Текст.ДобавитьСтроку("НомерАтАк:");
		
	Иначе
		
		Текст.ДобавитьСтроку("НомерАтАк:" + Лев(ВРЕГ(СокрЛП(мСтруктураМногостраничныхРазделов.Раздел1[0].Данные.АттСерия)), 10)
								+ "," + Лев(ВРЕГ(СокрЛП(мСтруктураМногостраничныхРазделов.Раздел1[0].Данные.АттНомер)), 10));
								
	КонецЕсли;
	Текст.ДобавитьСтроку("ДатаАтест:" + Формат(Страница.Данные.АттДата, "ДФ=dd.MM.yyyy; ДП="));
	Текст.ДобавитьСтроку("НаимОргАт:" + ВРЕГ(Лев(СокрЛП(Страница.Данные.АттВыдан), 1000)));
	...
	Для Каждого Страница Из мСтруктураМногостраничныхРазделов.Раздел1 Цикл
		...
		Пока ТекДанные.Свойство(мТаблицаСоставПоказателей[0].КодПоказателяПоСоставу + "_" + Инд, Врем) Цикл
			...
			Для Каждого Показатель Из мТаблицаСоставПоказателей Цикл
				...
				КонецЕсли;
				Если Показатель.КодПоказателяПоСоставу = "П000010001005" Тогда
					КодПоказателяПоСоставу = "П000010001105";
				ИначеЕсли  Показатель.КодПоказателяПоСоставу = "П000010001007" Тогда
					КодПоказателяПоСоставу = "П000010001107";
				ИначеЕсли  Показатель.КодПоказателяПоСоставу = "П000010001009" Тогда
					КодПоказателяПоСоставу = "П000010001109";
				ИначеЕсли  Показатель.КодПоказателяПоСоставу = "П000010001010" Тогда
					КодПоказателяПоСоставу = "П000010001110";
				Иначе
					КодПоказателяПоСоставу = Показатель.КодПоказателяПоСоставу;
				КонецЕсли;
				Если НЕ РегламентированнаяОтчетность.ПустоеЗначение(ТекДанные[КодПоказателяПоСоставу + "_" + Инд]) Тогда
					СтрокаПустая = Ложь;
					Прервать;
				КонецЕсли;
			КонецЦикла;
...
СтруктураГруппы1.Колонки.Добавить("П000010001004", ОписаниеТиповСтрока254);
СтруктураГруппы1.Колонки.Добавить("П000010001105", ОписаниеТиповСтрока15);
СтруктураГруппы1.Колонки.Добавить("П000010001205", ОписаниеТиповСтрока15);
СтруктураГруппы1.Колонки.Добавить("П000010001006", ОписаниеТиповСтрока254);
СтруктураГруппы1.Колонки.Добавить("П000010001107", ОписаниеТиповСтрока15);
СтруктураГруппы1.Колонки.Добавить("П000010001207", ОписаниеТиповСтрока15);
СтруктураГруппы1.Колонки.Добавить("П000010001307", ОписаниеТиповДата);
СтруктураГруппы1.Колонки.Добавить("П000010001008", ОписаниеТиповСтрока254);
СтруктураГруппы1.Колонки.Добавить("П000010001109", ОписаниеТиповСтрока15);
СтруктураГруппы1.Колонки.Добавить("П000010001209", ОписаниеТиповДата);
СтруктураГруппы1.Колонки.Добавить("П000010001309", ОписаниеТиповЧисло);
СтруктураГруппы1.Колонки.Добавить("П000010001110", ОписаниеТиповДата);
СтруктураГруппы1.Колонки.Добавить("П000010001210", ОписаниеТиповСтрока15);
СтруктураГруппы1.Колонки.Добавить("П000010001011", ОписаниеТиповСтрока15);
СтруктураГруппы1.Колонки.Добавить("П000010001012", ОписаниеТиповДата);
