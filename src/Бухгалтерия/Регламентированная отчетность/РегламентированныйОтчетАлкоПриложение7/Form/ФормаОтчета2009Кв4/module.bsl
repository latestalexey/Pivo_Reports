...
Процедура ПолеТабличногоДокументаДекларацияВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если ((Найти(Область.Имя, "П000000000003") = 1) ИЛИ (Найти(Область.Имя, "П000000000002") = 1))
...
Функция СформироватьСтруктуруПараметров()
	...
	Параметры.Вставить("ИмяФайлаСхемы",				"ALK_1152025_4_01.xsd");
	Параметры.Вставить("ПрефиксИмениФайлаВыгрузки",	"A7");
	...
	Пока СвойствоОпределено(РазделДекларация, "П000000000003_" + стрНомСтроки) Цикл
		Если ЗначениеЗаполнено(РазделДекларация["П000000000003_" + стрНомСтроки]) Тогда
			...
			Если СтруктураАдреса.Страна = "643" Тогда
				//это резидент
				УдалитьУзел(ПолучитьПодчиненныйЭлемент(УзелОбъемЗакупки, "Иностр"));
			...
			Иначе
				//это иностранец
				УдалитьУзел(ПолучитьПодчиненныйЭлемент(УзелОбъемЗакупки, "Резидент"));
...
СтруктураГруппы1.Колонки.Добавить("П000000000010", ОписаниеТиповСтрока254); // Кем выд.
...
СтруктураГруппы4.Колонки.Добавить("П000010001004", ОписаниеТиповСтрока1000); // номер
...
КНД = "1152025";
НаименованиеОтчетаДляВыгрузки = "ДЕКЛАРАЦИЯ ОБ ОБЪЕМАХ ЗАКУПКИ ЭТИЛОВОГО СПИРТА, АЛКОГОЛЬНОЙ И СПИРТОСОДЕРЖАЩЕЙ ПРОДУКЦИИ";
