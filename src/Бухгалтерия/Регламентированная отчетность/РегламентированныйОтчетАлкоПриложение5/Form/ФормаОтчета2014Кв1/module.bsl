...
Запрос = Новый Запрос;
	...
	|		ИЗ
	|			РегистрБухгалтерии.Хозрасчетный.Обороты(&ДатаНач, &ДатаКон, Регистратор, Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ТоварыНаСкладах), &ВидСубконто, Организация В (&СписокОрганизаций), КорСчет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПокупныеИзделия), ) КАК ХозрасчетныйОборотыОтветХранение) КАК ВложенныйЗапрос
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СведенияОбАлкогольнойПродукции КАК СведенияОбАлкогольнойПродукции
	...
	ТаблицаЗапроса.Сортировать("СсылкаПодразделение, П000010000101, П000010000102, П000010000103, П000010000113, П000010000115, П000010000106");
