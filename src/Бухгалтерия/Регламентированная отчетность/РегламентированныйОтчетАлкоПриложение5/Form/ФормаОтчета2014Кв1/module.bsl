...
Запрос = Новый Запрос;
	...
	|		КОНЕЦ,
	|		ХозрасчетныйОборотыНач.Организация,
	|		ХозрасчетныйОборотыНач.Подразделение) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.П000010000104,
	|	ВложенныйЗапрос.П000010000105,
	...
	ТаблицаЗаполнения23 = мСтруктураМногострочныхРазделов.П0000100003.СкопироватьКолонки();
	...
	Для Каждого Подразделение Из ДеревоПоПодразделениям.Строки Цикл
		...
		ВывестиРазделВТабличныйДокумент("П0000100003", ТаблицаЗаполнения23);
		Расчет("Декларация");
	КонецЦикла;
