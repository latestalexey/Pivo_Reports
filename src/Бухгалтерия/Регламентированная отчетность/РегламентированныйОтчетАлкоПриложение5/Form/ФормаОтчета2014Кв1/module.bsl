...
Запрос = Новый Запрос;
	...
	|		NULL,
	|		NULL,
	|		ХозрасчетныйОборотыНач.Подразделение,
	|		ХозрасчетныйОборотыНач.Организация
	|	ИЗ
	|		(ВЫБРАТЬ
	|			СведенияОбАлкогольнойПродукции.НаименованиеВида169 КАК П000010000101,
	|			СведенияОбАлкогольнойПродукции.КодВида169 КАК П000010000102,
	...
	|			ХозрасчетныйОстаткиНач.Подразделение) КАК ХозрасчетныйОборотыНач
	...
	|	ГДЕ
	|		ХозрасчетныйОборотыНач.П000010000106 <> 0
	|		И ХозрасчетныйОборотыНач.П000010000120 <> 0
	|	
	|	СГРУППИРОВАТЬ ПО
	|		СведенияОбАлкогольнойПродукции.НаименованиеВида169,
	|		СведенияОбАлкогольнойПродукции.КодВида169,
	...
	|	ИЗ
	|		РегистрСведений.СведенияОбАлкогольнойПродукции КАК СведенияОбАлкогольнойПродукции
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.Обороты(&ДатаНач, &ДатаКон, Регистратор, Счет В (&Счет41), &ВидСубконто, Организация В (&СписокОрганизаций), КорСчет В (&Счет41), ) КАК ХозрасчетныйОбороты
	|			ПО СведенияОбАлкогольнойПродукции.Номенклатура = ХозрасчетныйОбороты.Субконто1
	|	
	|	СГРУППИРОВАТЬ ПО
	|		СведенияОбАлкогольнойПродукции.НаименованиеВида169,
	...
	|		КОНЕЦ,
	|		ХозрасчетныйОбороты.Подразделение,
	|		ХозрасчетныйОбороты.Организация) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.П000010000104,
	|	ВложенныйЗапрос.П000010000105,
