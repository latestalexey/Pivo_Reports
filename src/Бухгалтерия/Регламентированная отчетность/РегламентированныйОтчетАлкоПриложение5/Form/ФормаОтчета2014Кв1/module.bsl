...
Функция ПолучитьВерсиюФормата()
	...
	Если мДатаКонцаПериодаОтчета < '20150901' Тогда		
		Результат = "430_2014Кв1";
	Иначе		
		Результат = "431";
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеДекларация(ПараметрыВыгрузки, СтатусПроверки)
	...
	Для Каждого ЛистРаздела Из ДанныеРаздела Цикл
		...
		Если НомерЛиста = 1 Тогда
			...
			КонецЕсли;
			КоличЗаполненных = 0;
			ПроверитьЗаполнениеТаблицыДекларации(СтатусПроверки, мИдГруппы1, ДанныеДопСтрок, ДополнительныеДанныеЛиста, НомерЛиста, КоличЗаполненных);
		...
		КонецЕсли;
		КоличЗаполненных = 0;
		ПроверитьЗаполнениеТаблицыДекларации(СтатусПроверки, мИдГруппы2, ДанныеДопСтрок, ДополнительныеДанныеЛиста, НомерЛиста, КоличЗаполненных);
...
#Область Запрос_оборотов //14.10.2016 11:03:32
	"ВЫБРАТЬ
	...
	|	ВложенныйЗапрос.П000010000106,
	|	ВложенныйЗапрос.П000010000107,
	|	ВложенныйЗапрос.П000010000108,
	|	ВложенныйЗапрос.П000010000109,
	|	ВложенныйЗапрос.П000010000111,
	|	ВложенныйЗапрос.П000010000112,
	|	ВложенныйЗапрос.П000010000113,
	|	ВложенныйЗапрос.П000010000115,
	|	ВложенныйЗапрос.П000010000116,
	|	ВложенныйЗапрос.П000010000119,
	|	ВложенныйЗапрос.П000010000120,
	|	ВложенныйЗапрос.П000010000121,
	|	ВложенныйЗапрос.П000010000123,
	...
	|		СУММА(ВложенныйЗапрос.П000010000106 * СведенияОбАлкогольнойПродукции.КоэффПересчетаДал) КАК П000010000106,
	|		СУММА(ВложенныйЗапрос.П000010000107 * СведенияОбАлкогольнойПродукции.КоэффПересчетаДал) КАК П000010000107,
	|		СУММА(ВложенныйЗапрос.П000010000108 * СведенияОбАлкогольнойПродукции.КоэффПересчетаДал) КАК П000010000108,
	|		СУММА(ВложенныйЗапрос.П000010000109 * СведенияОбАлкогольнойПродукции.КоэффПересчетаДал) КАК П000010000109,
	|		СУММА(ВложенныйЗапрос.П000010000111 * СведенияОбАлкогольнойПродукции.КоэффПересчетаДал) КАК П000010000111,
	|		СУММА(ВложенныйЗапрос.П000010000112 * СведенияОбАлкогольнойПродукции.КоэффПересчетаДал) КАК П000010000112,
	|		СУММА(ВложенныйЗапрос.П000010000113 * СведенияОбАлкогольнойПродукции.КоэффПересчетаДал) КАК П000010000113,
	|		СУММА(ВложенныйЗапрос.П000010000115 * СведенияОбАлкогольнойПродукции.КоэффПересчетаДал) КАК П000010000115,
	|		СУММА(ВложенныйЗапрос.П000010000116 * СведенияОбАлкогольнойПродукции.КоэффПересчетаДал) КАК П000010000116,
	|		СУММА(ВложенныйЗапрос.П000010000119 * СведенияОбАлкогольнойПродукции.КоэффПересчетаДал) КАК П000010000119,
	|		СУММА(ВложенныйЗапрос.П000010000120 * СведенияОбАлкогольнойПродукции.КоэффПересчетаДал) КАК П000010000120,
	|		СУММА(ВложенныйЗапрос.П000010000121 * СведенияОбАлкогольнойПродукции.КоэффПересчетаДал) КАК П000010000121,
	|		СУММА(ВложенныйЗапрос.П000010000123 * СведенияОбАлкогольнойПродукции.КоэффПересчетаДал) КАК П000010000123,
	...
	|			0 КАК П000010000106,
	|			ВЫБОР
	|				КОГДА ХозрасчетныйОборотыПокупкаПродажи.КорСчет В (&Счет60)
	|						И ХозрасчетныйОборотыПокупкаПродажи.КорСубконто1 = СведенияОбАлкогольнойПродукции.Номенклатура.Производитель
	|					ТОГДА ХозрасчетныйОборотыПокупкаПродажи.КоличествоОборотДт
	|				ИНАЧЕ 0
	...
	|			0 КАК П000010000109,
	|			ВЫБОР
	|				КОГДА ХозрасчетныйОборотыПокупкаПродажи.Регистратор ССЫЛКА Документ.ВозвратТоваровОтПокупателя
	|					ТОГДА ХозрасчетныйОборотыПокупкаПродажи.КоличествоОборотДт - ХозрасчетныйОборотыПокупкаПродажи.КоличествоОборотКт
	|				ИНАЧЕ 0
	|			КОНЕЦ КАК П000010000111,
	|			ВЫБОР
	|				КОГДА НЕ ХозрасчетныйОборотыПокупкаПродажи.КорСчет В (&Счет60)
	|						И НЕ ХозрасчетныйОборотыПокупкаПродажи.Регистратор ССЫЛКА Документ.ВозвратТоваровОтПокупателя
	|					ТОГДА ХозрасчетныйОборотыПокупкаПродажи.КоличествоОборотДт
	|				ИНАЧЕ 0
	|			КОНЕЦ КАК П000010000112,
	|			0 КАК П000010000113,
	|			ВЫБОР
	|				КОГДА НЕ ХозрасчетныйОборотыПокупкаПродажи.Регистратор ССЫЛКА Документ.ОтчетОРозничныхПродажах
	|						И НЕ ХозрасчетныйОборотыПокупкаПродажи.Регистратор ССЫЛКА Документ.ВозвратТоваровПоставщику
	|						И НЕ ХозрасчетныйОборотыПокупкаПродажи.Регистратор ССЫЛКА Документ.СписаниеТоваров
	|						И ХозрасчетныйОборотыПокупкаПродажи.КоличествоОборотКт > 0
	|					ТОГДА ХозрасчетныйОборотыПокупкаПродажи.КоличествоОборотКт
	|				ИНАЧЕ 0
	|			КОНЕЦ КАК П000010000115,
	|			ВЫБОР
	|				КОГДА ХозрасчетныйОборотыПокупкаПродажи.Регистратор ССЫЛКА Документ.ОтчетОРозничныхПродажах
	|					ТОГДА ХозрасчетныйОборотыПокупкаПродажи.КоличествоОборотКт
	|				ИНАЧЕ 0
	|			КОНЕЦ КАК П000010000116,
	|			ВЫБОР
	|				КОГДА ХозрасчетныйОборотыПокупкаПродажи.Регистратор ССЫЛКА Документ.СписаниеТоваров
	|					ТОГДА ХозрасчетныйОборотыПокупкаПродажи.КоличествоОборотКт
	|				ИНАЧЕ 0
	|			КОНЕЦ КАК П000010000119,
	|			ВЫБОР
	|				КОГДА ХозрасчетныйОборотыПокупкаПродажи.Регистратор ССЫЛКА Документ.ВозвратТоваровПоставщику
	|					ТОГДА ХозрасчетныйОборотыПокупкаПродажи.КоличествоОборотКт
	|				ИНАЧЕ 0
	|			КОНЕЦ КАК П000010000120,
	|			0 КАК П000010000121,
	|			0 КАК П000010000123,
	|			ХозрасчетныйОборотыПокупкаПродажи.Субконто1 КАК Номенклатура,
	|			ХозрасчетныйОборотыПокупкаПродажи.Подразделение КАК Подразделение,
	|			ХозрасчетныйОборотыПокупкаПродажи.Организация КАК Организация
	|		ИЗ
	|			РегистрБухгалтерии.Хозрасчетный.Обороты(&ДатаНач, &ДатаКон, Регистратор, Счет В (&Счет41), &ВидСубконто, Организация В (&СписокОрганизаций), , ) КАК ХозрасчетныйОборотыПокупкаПродажи
	|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СведенияОбАлкогольнойПродукции КАК СведенияОбАлкогольнойПродукции
	|				ПО ХозрасчетныйОборотыПокупкаПродажи.Субконто1 = СведенияОбАлкогольнойПродукции.Номенклатура
	|		ГДЕ
	|			НЕ ХозрасчетныйОборотыПокупкаПродажи.КорСчет ЕСТЬ NULL 
	|			И (НЕ ХозрасчетныйОборотыПокупкаПродажи.КорСчет В (&Счет41)
	|					ИЛИ ХозрасчетныйОборотыПокупкаПродажи.КорСчет ЕСТЬ NULL )
	|		
	|		ОБЪЕДИНИТЬ ВСЕ
	|		
	|		ВЫБРАТЬ
	|			ХозрасчетныйОстаткиНач.КоличествоОстатокДт,
	|			0,
	|			0,
	|			0,
	|			0,
	...
	|			0,
	|			0,
	|			ХозрасчетныйОстаткиНач.Субконто1,
	|			ХозрасчетныйОстаткиНач.Подразделение,
	|			ХозрасчетныйОстаткиНач.Организация
	|		ИЗ
	|			РегистрБухгалтерии.Хозрасчетный.Остатки(&ДатаНач, Счет В (&Счет41), &ВидСубконто, Организация В (&СписокОрганизаций)) КАК ХозрасчетныйОстаткиНач
	|		
	|		ОБЪЕДИНИТЬ ВСЕ
	|		
	|		ВЫБРАТЬ
	...
	|			0,
	|			0,
	|			ХозрасчетныйОстаткиКон.КоличествоОстатокДт,
	|			ХозрасчетныйОстаткиКон.Субконто1,
	|			ХозрасчетныйОстаткиКон.Подразделение,
	|			ХозрасчетныйОстаткиКон.Организация
	|		ИЗ
	|			РегистрБухгалтерии.Хозрасчетный.Остатки(&ДатаКон, Счет В (&Счет41), &ВидСубконто, Организация В (&СписокОрганизаций)) КАК ХозрасчетныйОстаткиКон
	|		
	|		ОБЪЕДИНИТЬ ВСЕ
	|		
	|		ВЫБРАТЬ
	...
	|			0,
	|			ВЫБОР
	|				КОГДА ХозрасчетныйОборотыОтветХранение.Регистратор ССЫЛКА Документ.ПеремещениеТоваров
	|						И ХозрасчетныйОборотыОтветХранение.Регистратор.Контрагент <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|						И ХозрасчетныйОборотыОтветХранение.Регистратор.ЭтоВозврат
	|					ТОГДА ХозрасчетныйОборотыОтветХранение.КоличествоОборотДт
	|				ИНАЧЕ 0
	|			КОНЕЦ,
	|			0,
	|			0,
	...
	|				КОГДА ХозрасчетныйОборотыОтветХранение.Регистратор ССЫЛКА Документ.ПеремещениеТоваров
	|						И ХозрасчетныйОборотыОтветХранение.Регистратор.Контрагент <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|						И НЕ ХозрасчетныйОборотыОтветХранение.Регистратор.ЭтоВозврат
	|						И ХозрасчетныйОборотыОтветХранение.КоличествоОборотКт > 0
	|					ТОГДА ХозрасчетныйОборотыОтветХранение.КоличествоОборотКт
	|				ИНАЧЕ 0
	|			КОНЕЦ,
	|			0,
	|			0,
	|			0,
	|			ХозрасчетныйОборотыОтветХранение.Субконто1,
	|			ХозрасчетныйОборотыОтветХранение.Подразделение,
	|			ХозрасчетныйОборотыОтветХранение.Организация
	|		ИЗ
	|			РегистрБухгалтерии.Хозрасчетный.Обороты(&ДатаНач, &ДатаКон, Регистратор, Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ТоварыНаСкладах), &ВидСубконто, Организация В (&СписокОрганизаций), КорСчет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПокупныеИзделия), ) КАК ХозрасчетныйОборотыОтветХранение
	|		
	|		ОБЪЕДИНИТЬ ВСЕ
	|		
	|		ВЫБРАТЬ
	...
	|			0,
	|			0,
	|			ХозрасчетныйОборотыОтветХранение.КоличествоОборотДт,
	|			0,
	|			0,
	|			0,
	|			ХозрасчетныйОборотыОтветХранение.КоличествоОборотКт,
	|			0,
	|			0,
	|			0,
	|			ХозрасчетныйОборотыОтветХранение.Субконто1,
	...
	|			ХозрасчетныйОборотыОтветХранение.Организация
	|		ИЗ
	|			РегистрБухгалтерии.Хозрасчетный.Обороты(&ДатаНач, &ДатаКон, Регистратор, Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ТМЦпринятыеНаОтветственноеХранение), &ВидСубконто, Организация В (&СписокОрганизаций), , ) КАК ХозрасчетныйОборотыОтветХранение) КАК ВложенныйЗапрос
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СведенияОбАлкогольнойПродукции КАК СведенияОбАлкогольнойПродукции
	|			ПО ВложенныйЗапрос.Номенклатура = СведенияОбАлкогольнойПродукции.Номенклатура
	|	
	|	СГРУППИРОВАТЬ ПО
...
#КонецОбласти
	...
	Для Каждого СтрокаЗапроса Из ТаблицаЗапроса Цикл
		...
		КонецЕсли;
	КонецЦикла;
	СтрокаСортировки	="П000010000102, П000010000103";
	СтрокаСверткиПоля	="П000010000101,П000010000102,П000010000103,П000010000104,П000010000105";
	СтрокаСверткиДанные	="П000010000106,П000010000107,П000010000108,П000010000109,П000010000111,П000010000112,П000010000113,П000010000115,П000010000116,П000010000119,П000010000120,П000010000121,П000010000123";
	
	ТаблицаПоОрганизации = ТаблицаЗапроса.Скопировать();
	ТаблицаПоОрганизации.Свернуть(СтрокаСверткиПоля, СтрокаСверткиДанные);
	ТаблицаПоОрганизации.Сортировать(СтрокаСортировки);
	
	ТаблицаЗапроса.Свернуть(СтрокаСверткиПоля+",СсылкаПодразделение", СтрокаСверткиДанные);
