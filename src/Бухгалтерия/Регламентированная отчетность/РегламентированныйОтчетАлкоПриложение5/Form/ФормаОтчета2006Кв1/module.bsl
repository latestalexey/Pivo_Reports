...
Процедура Расчет(ИмяТекущейСтраницыПанели = "", КолонкаИзменений = Неопределено, НомерИзменяемойСтроки = Неопределено) Экспорт
	...
	Для Ном = НачальныйНомер По КонечныйНомер Цикл
		...
		мДекларация.Области["П000010001006" + НомСтр].Значение = 
		мДекларация.Области["П000010001007" + НомСтр].Значение +
		мДекларация.Области["П000010001008" + НомСтр].Значение +
		мДекларация.Области["П000010001009" + НомСтр].Значение;

		ТекСтрокаТаблицы = СтруктураГруппы[Ном - 1];
		ТекСтрокаТаблицы.П000010001006 = мДекларация.Области["П000010001006" + НомСтр].Значение;

		_5яКолонка = мДекларация.Области["П000010001006" + НомСтр].Значение
		   + мДекларация.Области["П000010001010" + НомСтр].Значение;

		ТекСтрокаТаблицы.П000010001005 = _5яКолонка;
		мДекларация.Области["П000010001005" + НомСтр].Значение = _5яКолонка;

		мДекларация.Области["П000010001012" + НомСтр].Значение = 
		мДекларация.Области["П000010001013" + НомСтр].Значение +
		мДекларация.Области["П000010001014" + НомСтр].Значение +
		мДекларация.Области["П000010001015" + НомСтр].Значение;

		ТекСтрокаТаблицы.П000010001012 = мДекларация.Области["П000010001012" + НомСтр].Значение;

		_11яКолонка = мДекларация.Области["П000010001012" + НомСтр].Значение
		   + мДекларация.Области["П000010001016" + НомСтр].Значение
		   + мДекларация.Области["П000010001017" + НомСтр].Значение;

		ТекСтрокаТаблицы.П000010001011 = _11яКолонка;
		мДекларация.Области["П000010001011" + НомСтр].Значение = _11яКолонка;

		мДекларация.Области["П000010001018" + НомСтр].Значение = мДекларация.Области["П000010001004" + НомСтр].Значение
		   + мДекларация.Области["П000010001005" + НомСтр].Значение
		   - мДекларация.Области["П000010001011" + НомСтр].Значение;

		ТекСтрокаТаблицы.П000010001018 = мДекларация.Области["П000010001018" + НомСтр].Значение;
...
СтруктураГруппы1.Колонки.Добавить("П000010001004", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14, 4)); // Остаток
СтруктураГруппы1.Колонки.Добавить("П000010001005", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14, 4)); // Всего
СтруктураГруппы1.Колонки.Добавить("П000010001006", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14, 4)); // От орг. производ этил. спирта
СтруктураГруппы1.Колонки.Добавить("П000010001007", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14, 4)); // Собственного пр-ва
СтруктураГруппы1.Колонки.Добавить("П000010001008", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14, 4)); // По импорту
СтруктураГруппы1.Колонки.Добавить("П000010001009", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14,4)); // Прочие поступления
СтруктураГруппы1.Колонки.Добавить("П000010001010", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14, 4)); // Всего
СтруктураГруппы1.Колонки.Добавить("П000010001011", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14, 4)); // Алкогольной продукции
СтруктураГруппы1.Колонки.Добавить("П000010001012", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14, 4)); // Денатур. этил. спирта
СтруктураГруппы1.Колонки.Добавить("П000010001013", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14, 4)); // Пищевой
СтруктураГруппы1.Колонки.Добавить("П000010001014", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14, 4)); // НЕпищевой
СтруктураГруппы1.Колонки.Добавить("П000010001015", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14, 4)); // прочий расход, в том числе возврат поставщику
СтруктураГруппы1.Колонки.Добавить("П000010001016", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14, 4)); // Остаток на конец О.П.
СтруктураГруппы1.Колонки.Добавить("П000010001017", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14, 4)); // Остаток на конец О.П.
СтруктураГруппы1.Колонки.Добавить("П000010001018", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14, 4)); // Остаток на конец О.П.
