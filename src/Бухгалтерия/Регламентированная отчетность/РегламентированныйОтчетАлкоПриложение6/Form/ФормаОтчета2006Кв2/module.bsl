...
Процедура ОбновитьСтруктуруМногострочногоРаздела(ИмяГруппы) Экспорт
	...
	Для НСтр = 1 По СтруктураГруппы.Количество() Цикл
		...
		Для Каждого Колонка Из СтруктураГруппы.Колонки Цикл
			...
			Если (ИмяКолонки = "П000000000008") Или (ИмяКолонки = "П000010003008") Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ВставитьДополнительнуюСтроку(Группа = Неопределено)
	...
	Ч2Верх  = ТекТабличноеПоле.Области.МногострочнаяЧасть2.Верх + ?(ИмяГруппы = мИдГруппы1, 4, 0);
	Ч2Низ   = ТекТабличноеПоле.Области.МногострочнаяЧасть2.Низ + ?(ИмяГруппы = мИдГруппы1, 4, 0);
	...
	СледующаяЧасть1 = ТекТабличноеПоле.Область(Ч1ВерхОпределениеВысотыСтрокиМногострочнойЧасти, 242, Ч1НизОпределениеВысотыСтрокиМногострочнойЧасти, 242);
...
Процедура УдалитьДополнительнуюСтроку()
	...
	Если ИмяГруппы = мИдГруппы2 И СтруктураГруппы.Количество() = 1 Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из СтруктураГруппы Цикл
		Для Каждого КолонкаТаблицы Из СтруктураГруппы.Колонки Цикл
			...
			Если ОбластьЯчейкиТаблицы.СодержитЗначение = Истина Тогда
				...
				Если (КолонкаТаблицы.Имя = "П000000000008" Или КолонкаТаблицы.Имя = "П000010003008") Тогда
					ЗначениеЯчейки = РегламентированнаяОтчетность.ПредставлениеАдресаВФормате9Запятых(ЗначениеЯчейки, Истина);
				КонецЕсли;

				ОбластьЯчейкиТаблицы.Значение = ЗначениеЯчейки;

				// Обеспечим маску ввода ИННа 12 символов.
				Если Найти(ОбластьЯчейкиТаблицы.Имя, "П000000000006") > 0 Тогда
					ОбластьЯчейкиТаблицы.ЭлементУправления.Маска = "999999999999";
				КонецЕсли;

				// Обеспечим маску ввода КПП 9 символов.
				Если Найти(ОбластьЯчейкиТаблицы.Имя, "П000000000007") > 0 Тогда
...
Процедура ОчиститьТребуемуюГруппу(Группа, ВыбТабличноеПоле)
	...
	Если ИмяГруппыДопСтрок = мИдГруппы1 Или ИмяГруппыДопСтрок = мИдГруппы2 Тогда
	...
	Для Инд = 0 По ВыбТабличноеПоле.Области.Количество() - 1 Цикл
		...
		Если (ИмяГруппыДопСтрок = мИдГруппы1 И Найти(ТекущаяОбласть.Имя, "П000010003008"))
		 Или (ИмяГруппыДопСтрок = мИдГруппы2 И Найти(ТекущаяОбласть.Имя, "П000000000008")) Тогда
			Продолжить;
		КонецЕсли;

		Если Найти(ТекущаяОбласть.Имя, "П000000000008") > 0
		   Или Найти(ТекущаяОбласть.Имя, "П000010003008") >0 Тогда
			...
			Если ИмяГруппыДопСтрок = мИдГруппы1 Тогда
				СтруктураГруппы[Число(НомерСтроки) - 1].П000000000008 = "";
			ИначеЕсли ИмяГруппыДопСтрок = мИдГруппы2 Тогда
				СтруктураГруппы[Число(НомерСтроки) - 1].П000010003008 = "";
			КонецЕсли;
		КонецЕсли;
...
Процедура СформироватьПечатныеФормыРазделаДекларация(СтруктураПараметровСтраницы)
	...
	Если ПриПечатиГруппироватьПоКодам Тогда
		...
		ТаблицаСорт.Сортировать("П000000000003, П000000000002, П000000000004");
		ТаблицаСвер = ТаблицаСорт.Скопировать();
		ТаблицаСвер.Свернуть("П000000000003");
		...
		ТаблицаДляОрганизации.Сортировать("П000000000004");
		...
		Для каждого Код Из ТаблицаСвер Цикл
			Если ПустаяСтрока(Код.П000000000003) Тогда
				КолвоПустых = КолвоПустых + 1;
				Продолжить;
			КонецЕсли;
			ДанныеПоКоду = ТаблицаСорт.НайтиСтроки(Новый Структура("П000000000003", Код.П000000000003));
			...
			ТекТабличноеПоле.Вывести(ЭлементыФормы.ПолеТабличногоДокументаДекларация.ПолучитьОбласть(1, 0, ЭлементыФормы.ПолеТабличногоДокументаДекларация.Области.ОсновнаяЧасть2.Низ, 0));

			//ТекТабличноеПоле.Вывести(ЭлементыФормы.ПолеТабличногоДокументаДекларация.ПолучитьОбласть(ЭлементыФормы.ПолеТабличногоДокументаДекларация.Области.ПустаяСтрока.Верх, 0, ЭлементыФормы.ПолеТабличногоДокументаДекларация.ВысотаТаблицы, 0));
			ТекТабличноеПоле.Вывести(ЭлементыФормы.ПолеТабличногоДокументаДекларация.ПолучитьОбласть(ЭлементыФормы.ПолеТабличногоДокументаДекларация.Области.ОсновнаяЧасть3.Верх, 0, ЭлементыФормы.ПолеТабличногоДокументаДекларация.ВысотаТаблицы, 0));
			...
			Для каждого Строчка Из ДанныеПоКоду Цикл
				...
				ТекТабличноеПоле.Области["П000000000001_" + СчетСтр].Текст = СчетСтр;
				ТекТабличноеПоле.Области["П000000000002_" + СчетСтр].Значение = Строчка.П000000000002;
				ТекТабличноеПоле.Области["П000000000003_" + СчетСтр].Значение = Строчка.П000000000003;
				ТекТабличноеПоле.Области["П000000000004_" + СчетСтр].Значение = Строчка.П000000000004;
				ТекТабличноеПоле.Области["П000000000005_" + СчетСтр].Значение = Строчка.П000000000005;
				ТекТабличноеПоле.Области["П000000000006_" + СчетСтр].Значение = Строчка.П000000000006;
				ТекТабличноеПоле.Области["П000000000007_" + СчетСтр].Значение = Строчка.П000000000007;
				ТекТабличноеПоле.Области["П000000000008_" + СчетСтр].Значение = РегламентированнаяОтчетность.ПредставлениеАдресаВФормате9Запятых(Строчка.П000000000008, Истина);

				ТекТабличноеПоле.Области["П000000001109_" + СчетСтр].Значение = Строчка.П000000001109;
				ТекТабличноеПоле.Области["П000000001209_" + СчетСтр].Значение = Строчка.П000000001209;
				ТекТабличноеПоле.Области["П000000001309_" + СчетСтр].Значение = Строчка.П000000001309;
				ТекТабличноеПоле.Области["П000000001409_" + СчетСтр].Значение = Строчка.П000000001409;

				ТекТабличноеПоле.Области["П000000000010_" + СчетСтр].Значение = Строчка.П000000000010;
				ТекТабличноеПоле.Области["П000000001111_" + СчетСтр].Значение = Строчка.П000000001111;
				ТекТабличноеПоле.Области["П000000001211_" + СчетСтр].Значение = Строчка.П000000001211;

				ТекТабличноеПоле.Области["П000000000012_" + СчетСтр].Значение = Строчка.П000000000012;
				ТекТабличноеПоле.Области["П000000000013_" + СчетСтр].Значение = Строчка.П000000000013;
				ТекТабличноеПоле.Области["П000000000014_" + СчетСтр].Значение = Строчка.П000000000014;
				ТекТабличноеПоле.Области["П000000000015_" + СчетСтр].Значение = Строчка.П000000000015;
				ТекТабличноеПоле.Области["П000000000016_" + СчетСтр].Значение = Строчка.П000000000016;
				СуммаКолонка16 = СуммаКолонка16 + Строчка.П000000000016;
				Счетчик = Счетчик + 1;
			КонецЦикла;

			// Выведем итог по коду продукции.
			ТекТабличноеПоле.Области.П000010002016.Значение = СуммаКолонка16;
			ТекТабличноеПоле.Области.ИтогВсего.Текст = "Итог:";

			ТаблицаПриемник = СтруктураТаблОрг.Скопировать();
			ОставитьВТаблицеТребуемыйКодПродукции(ТаблицаДляОрганизации, ТаблицаПриемник, "П000000000003", Код.П000000000003);
			ТаблицаПриемник.Свернуть("П000000000006, П000000000007, П000000000004", "П000000000016");
			ТаблицаПриемник.Сортировать("П000000000004");
			...
			Если ТребуемоеКолВоСтрокВторойМнгЧ > КоличествоСтрокВторойМнЧ Тогда
				...
				Для Инд = КоличествоСтрокВторойМнЧ + 1 По ТребуемоеКолВоСтрокВторойМнгЧ Цикл
					...
					КолСтрок = Инд - 1;
					ИмяПервойЯчейкиПоследнейСтроки = "П000010003001" + "_" + Формат(Инд - 1, "ЧГ=0");
					// область первой ячейки последней строки многострочного раздела
					ОбластьЯчейки = ТекТабличноеПоле.Области[ИмяПервойЯчейкиПоследнейСтроки];
			...
			Для каждого Контрагент Из ТаблицаПриемник Цикл
				...
				ТекТабличноеПоле.Области["П000010003001_" + ИндС].Текст = ИндС;

				Данные = ПолучитьЗначениеИзОсновнойТаблице(Контрагент.П000000000006, Контрагент.П000000000007, СтруктураГруппы1, Контрагент.П000000000004);

				ТекТабличноеПоле.Области["П000010003004_" + ИндС].Значение = Данные.Наименование;
				ТекТабличноеПоле.Области["П000010003005_" + ИндС].Значение = Данные.Регион;
				ТекТабличноеПоле.Области["П000010003006_" + ИндС].Значение = Контрагент.П000000000006;
				ТекТабличноеПоле.Области["П000010003007_" + ИндС].Значение = Контрагент.П000000000007;
				ТекТабличноеПоле.Области["П000010003008_" + ИндС].Значение = РегламентированнаяОтчетность.ПредставлениеАдресаВФормате9Запятых(Данные.Адрес, Истина);
				ТекТабличноеПоле.Области["П000010003016_" + ИндС].Значение = Контрагент.П000000000016;
				Счетчик = Счетчик + 1;
		...
		Если КолвоПустых = ТаблицаСвер.Количество() Тогда
			Сообщить("Нет сведений для печати листа ""Декларация"" в особом режиме.", СтатусСообщения.Информация);
		...
		ЭлементыФормы.ПолеТабличногоДокументаВремТаблДок.Вывести(ЭлементыФормы.ПолеТабличногоДокументаДекларация.ПолучитьОбласть(ЭлементыФормы.ПолеТабличногоДокументаДекларация.Области.ОсновнаяЧасть3.Верх, 0, ЭлементыФормы.ПолеТабличногоДокументаДекларация.Области.ПустаяСтрока.Верх, 0));
		ТекТабличноеПоле.Области.ОсновнаяЧасть3.Имя = "ОсновнаяЧасть21";
		// Нужно основную часть2 вставить
		ЭлементыФормы.ПолеТабличногоДокументаВремТаблДок.Вывести(ЭлементыФормы.ПолеТабличногоДокументаДекларация.ПолучитьОбласть(ЭлементыФормы.ПолеТабличногоДокументаДекларация.Области.ОсновнаяЧасть2.Верх, 0, ЭлементыФормы.ПолеТабличногоДокументаДекларация.Области.ОсновнаяЧасть2.Низ, 0));
		...
		ЭлементыФормы.ПолеТабличногоДокументаВремТаблДок.Вывести(ЭлементыФормы.ПолеТабличногоДокументаДекларация.ПолучитьОбласть(ЭлементыФормы.ПолеТабличногоДокументаДекларация.Области.Ограничители.Верх, 0, ЭлементыФормы.ПолеТабличногоДокументаДекларация.Области.Ограничители.Низ, 0));
		ТекТабличноеПоле.Области.П000010002016.Значение = мДекларация.Области.П000010002016.Значение;
		ВывестиНаПечатьЛист(СтруктураПараметровСтраницы);
...
Функция Сохранить(Автосохранение = Ложь) Экспорт
	...
	КодИФНС = мТитульный.Области.КодИМНС1.Значение
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Для Каждого ПоказателиСтраницы Из ПоказателиОтчета Цикл
		...
		Для Каждого Показатель Из ПоказателиТаблПоле Цикл
			...
			Если Найти(ИмяПоказателя, "П000000000008") > 0
			   Или Найти(ИмяПоказателя, "П000010003008") > 0 Тогда
				Продолжить;
			КонецЕсли;
	...
	мАдрЮРТитульныйВыгрузка = мТитульный.Области.АдрЮР.Значение;
...
//
Процедура ОставитьВТаблицеТребуемыйКодПродукции(ТаблицаИсточник, ТаблицаПриемник, Поле, Условие) Экспорт

	// Заполним значения в совпадающих колонках.
	Для каждого СтрокаТаблицыИсточника Из ТаблицаИсточник Цикл

		Если (СтрокаТаблицыИсточника[Поле] = Условие)
		  И  (НЕ ПустаяСтрока(СтрокаТаблицыИсточника.П000000000006) )
		  И  (НЕ ПустаяСтрока(СтрокаТаблицыИсточника.П000000000006) )  Тогда
...
Процедура ЗаполнитьАвто()
	...
	ТаблицаРаздела.Свернуть("П000000000006, П000000000007, П000000000004", "П000000000016");

	// Удалим строки 2-й группы.
	УдалитьСтроки(мИдГруппы2, мДекларация);
	// Обновим раздел.
	...
	Для Каждого Стр Из ТаблицаРаздела Цикл
		...
		Если НЕ ЗначениеЗаполнено(Стр.П000000000006) И НЕ ЗначениеЗаполнено(Стр.П000000000007) И НЕ ЗначениеЗаполнено(Стр.П000000000004) Тогда
			Продолжить;
		КонецЕсли;
		...
		Данные = ПолучитьЗначениеИзОсновнойТаблице(Стр.П000000000006, Стр.П000000000007, СтруктураГруппы1);

		мДекларация.Области["П000010003004_" + ИндС].Значение = Данные.Наименование;
		мДекларация.Области["П000010003005_" + ИндС].Значение = Данные.Регион;
		мДекларация.Области["П000010003006_" + ИндС].Значение = Стр.П000000000006;
		мДекларация.Области["П000010003007_" + ИндС].Значение = Стр.П000000000007;
		СтруктураГруппы2[Инд - 1].П000010003008 = Данные.Адрес;
		мДекларация.Области["П000010003008_" + ИндС].Значение = РегламентированнаяОтчетность.ПредставлениеАдресаВФормате9Запятых(Данные.Адрес, Истина);
		мДекларация.Области["П000010003016_" + ИндС].Значение = Стр.П000000000016;
...
//  КПП            - ИНН организации
//
Функция ПолучитьЗначениеИзОсновнойТаблице(Знач ИНН, Знач КПП, СтруктураГруппы1, ИмяОрганизации = Неопределено)

	Итог = Новый Структура("Наименование, Регион, Адрес", "", "", "");
	ПоисковаяСтруктура = Новый Структура();
	ПоисковаяСтруктура.Вставить("П000000000006", ИНН);
	ПоисковаяСтруктура.Вставить("П000000000007", КПП);
	Если ИмяОрганизации <> Неопределено Тогда
		ПоисковаяСтруктура.Вставить("П000000000004", ИмяОрганизации);
	КонецЕсли;
	Результат = СтруктураГруппы1.НайтиСтроки(ПоисковаяСтруктура);

	Если Результат <> Неопределено Тогда
		Итог.Вставить("Адрес", Результат[0].П000000000008);
		Итог.Вставить("Регион", Результат[0].П000000000005);
		Итог.Вставить("Наименование", Результат[0].П000000000004);
		Возврат Итог;
	КонецЕсли;
	Возврат Итог;
			...
			Для НСтр = 1 По  ИсходноеКоличествоСтрокГруппы Цикл
				...
				Для Каждого Колонка Из СтруктураГруппы.Колонки Цикл
					...
					Если ОбластьЯчейки.СодержитЗначение = Истина Тогда
						...
						// Обеспечим маску ввода ИННа 12 символов.
						Если Найти(ОбластьЯчейки.Имя, "П000000000006") > 0 Тогда
							ОбластьЯчейки.ЭлементУправления.Маска = "999999999999";
						КонецЕсли;

						// Обеспечим маску ввода КПП 9 символов.
						Если Найти(ОбластьЯчейки.Имя, "П000000000007") > 0 Тогда
...
Функция ОбновитьПеременныеМодуля(ИмяПоказателя, ЗначениеПоказателя) Экспорт
	...
	Если ИмяПоказателя = "АдрЮРТитульныйВыгрузка" Тогда
...
Процедура ПолеТабличногоДокументаФизЛицаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если (Найти(Область.Имя, "ФизЛицИнд") > 0)
	   ...
	   Или (Область.Имя = "ФизЛицКвартира") Тогда
		...
		СтруктураАдресныхДанных.Вставить("НаселенныйПункт", "");
...
Процедура ПолеТабличногоДокументаПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если НЕ ((Страница = "Титульный") Или (Страница = "ФизЛица")) Тогда
		...
		Если Найти(ОбластьИмя, "П000000000016_") > 0 Тогда
			...
			Расчет("");
		ИначеЕсли (Найти(ОбластьИмя, "П000000000008_") > 0 Или Найти(ОбластьИмя, "П000010003008_") > 0) И (мАвтоВыборКодов) Тогда
...
мСоставПоказателей = "СоставПоказателей2006Кв2";
...
СтруктураГруппы1.Колонки.Добавить("П000000000001",ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(6)); // номер
СтруктураГруппы1.Колонки.Добавить("П000000000002", ОписаниеТиповСтрока254); // наименование продукции
СтруктураГруппы1.Колонки.Добавить("П000000000003",ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(3)); // код
СтруктураГруппы1.Колонки.Добавить("П000000000004", ОписаниеТиповСтрока254); // Получатель
...
СтруктураГруппы1.Колонки.Добавить("П000000001109",ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(15)); // Серия
СтруктураГруппы1.Колонки.Добавить("П000000001209", ОписаниеТиповСтрока254); // номер
...
СтруктураГруппы1.Колонки.Добавить("П000000001111", ОписаниеТиповСтрока254); // Номер уведомления
...
СтруктураГруппы1.Колонки.Добавить("П000000000013", ОписаниеТиповСтрока254); // номер ТТС
СтруктураГруппы1.Колонки.Добавить("П000000000014", ОписаниеТиповСтрока254); // номер груз. там. декл.
СтруктураГруппы1.Колонки.Добавить("П000000000015", ОписаниеТиповДата); // номер груз. там. декл.
СтруктураГруппы1.Колонки.Добавить("П000000000016", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14, 4));

СтруктураГруппы2 = Новый ТаблицаЗначений;
СтруктураГруппы2.Колонки.Добавить("П000010003001",ОбщегоНазначения.ПолучитьОписаниеТиповСтроки(6)); // номер
СтруктураГруппы2.Колонки.Добавить("П000010003004", ОписаниеТиповСтрока254); // номер
...
СтруктураГруппы2.Колонки.Добавить("П000010003016", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14, 4));
...
ВариантыФорматовВыгрузки.Добавить("СоставПоказателей2006Кв2", "Формат 3.00", Истина);
