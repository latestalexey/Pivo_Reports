...
Процедура ЗаполнитьАвтоБухгалтерия(СписокОрганизаций) Экспорт
	...
	Запрос.Текст =
#Область Запрос_оборотов //14.10.2016 10:46:01
	"ВЫБРАТЬ
	...
	|	ДанныеОборотов.Грузополучатель.ИНН КАК П000010000108,
	|	ДанныеОборотов.Грузополучатель.КПП КАК П000010000109,
	|	"""" КАК П000010000110,
	|	"""" КАК П000010000111,
	|	"""" КАК П000010000113,
	|	"""" КАК П000010000114,
	...
	|		ВЫБОР
	|			КОГДА Продажи.Регистратор.Грузополучатель ЕСТЬ НЕ NULL 
	|					И Продажи.Регистратор.Контрагент.КПП <> Продажи.Регистратор.Грузополучатель.КПП
	|				ТОГДА Продажи.Регистратор.Грузополучатель
	|			ИНАЧЕ Продажи.Регистратор.Контрагент
	|		КОНЕЦ КАК Грузополучатель,
	|		Продажи.Регистратор.Контрагент КАК Контрагент,
	|		ВЫБОР
	|			КОГДА Продажи.Субконто1.Импортер = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|				ТОГДА Продажи.Субконто1.Производитель
	|			ИНАЧЕ Продажи.Субконто1.Импортер
	|		КОНЕЦ КАК ПроизводительИмпортер
	|	ИЗ
	|		РегистрБухгалтерии.Хозрасчетный.Обороты(&ДатаНач, &ДатаКон, Регистратор, Счет В ИЕРАРХИИ (&Счет41), &ВидСубконто, Организация В (&СписокОрганизаций), КорСчет В ИЕРАРХИИ (&КорСчет), ) КАК Продажи
	|	ГДЕ
	|		Продажи.КоличествоОборотКт > 0
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		ПередачаНаОХ.Регистратор,
	|		ПередачаНаОХ.Субконто1,
	|		ПередачаНаОХ.Организация,
	|		ПередачаНаОХ.Подразделение,
	|		ПередачаНаОХ.КоличествоОборотДт,
	|		ПередачаНаОХ.Регистратор.Контрагент,
	|		ПередачаНаОХ.Регистратор.Контрагент,
	|		ВЫБОР
	|			КОГДА ПередачаНаОХ.Субконто1.Импортер = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|				ТОГДА ПередачаНаОХ.Субконто1.Производитель
	|			ИНАЧЕ ПередачаНаОХ.Субконто1.Импортер
	|		КОНЕЦ
	|	ИЗ
	|		РегистрБухгалтерии.Хозрасчетный.Обороты(&ДатаНач, &ДатаКон, Регистратор, Счет В ИЕРАРХИИ (&Счет41), &ВидСубконто, Организация В (&СписокОрганизаций), КорСчет В ИЕРАРХИИ (&Счет41), ) КАК ПередачаНаОХ
	|	ГДЕ
	|		ПередачаНаОХ.Регистратор.Контрагент <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|		И НЕ ПередачаНаОХ.Регистратор.ЭтоВозврат
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		ОборотыПоОХ.Регистратор,
	|		ОборотыПоОХ.Субконто1,
	|		ОборотыПоОХ.Организация,
	|		ОборотыПоОХ.Подразделение,
	|		ОборотыПоОХ.КоличествоОборотКт,
	|		ОборотыПоОХ.Регистратор.Контрагент,
	|		ОборотыПоОХ.Регистратор.Контрагент,
	|		ВЫБОР
	|			КОГДА ОборотыПоОХ.Субконто1.Импортер = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|				ТОГДА ОборотыПоОХ.Субконто1.Производитель
	|			ИНАЧЕ ОборотыПоОХ.Субконто1.Импортер
	|		КОНЕЦ
	|	ИЗ
	|		РегистрБухгалтерии.Хозрасчетный.Обороты(&ДатаНач, &ДатаКон, Регистратор, Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ТоварыНаСкладах), &ВидСубконто, Организация В (&СписокОрганизаций), КорСчет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПокупныеИзделия), ) КАК ОборотыПоОХ
	|	ГДЕ
	|		ОборотыПоОХ.КоличествоОборотКт > 0) КАК ДанныеОборотов
	...
	|	ДанныеОборотов.Организация,
	|	ДанныеОборотов.Грузополучатель.ИНН,
	|	ДанныеОборотов.Грузополучатель.КПП,
...
#КонецОбласти
	...
	Для Каждого СтрокаЗапроса Из ТаблицаЗапроса Цикл
		...
		КонецЕсли;
	КонецЦикла;
	СтрокаСортировки="П000010000102, П000010000103, П000010000106, П000010000117, П000010000118";
	ТаблицаЗапроса.Сортировать(СтрокаСортировки);
		
	ТаблицаПоОрганизации = ТаблицаЗапроса.Скопировать();
	ТаблицаПоОрганизации.Сортировать(СтрокаСортировки);
	...
	Для Каждого СтрокаЗапроса Из ТаблицаПоОрганизации Цикл
		...
		Если НЕ ДополнительныеДанныеСтраницы.Свойство(ИмяЭлементаСтруктуры+"_"+Формат(НС,"ЧГ=0"), СведенияОбПоставщике) ИЛИ СведенияОбПоставщике = Неопределено Тогда
			СведенияОбПоставщике = ИнициализацияСтруктурыСведенияОПоставщике();
		КонецЕсли;
		СтрокаОшибок="";
		СведенияОбПоставщике=ПолучитьСведенияОбАдресеКонтрагнета(СтрокаЗапроса.Контрагент.Ссылка,СтрокаОшибок);
	...
	Для Каждого Подразделение Из ДеревоПоПодразделениям.Строки Цикл
		...
		Для Каждого СтрокаПодразделения Из Подразделение.Строки Цикл
			...
			КонецЕсли;
			СтрокаОшибок="";
			СведенияОбПоставщике=ПолучитьСведенияОбАдресеКонтрагнета(СтрокаПодразделения.Контрагент.Ссылка,СтрокаОшибок);
			Если ЗначениеЗаполнено(СтрокаОшибок) Тогда
	...
	Запрос.Текст =
#Область Запрос_возвратов //14.10.2016 10:47:29
	"ВЫБРАТЬ
	...
	|	ВЫРАЗИТЬ(ВозвратыОборот.ПроизводительИмпортер.НаименованиеПолное КАК СТРОКА(200)) КАК П000020000103,
	|	ВозвратыОборот.ПроизводительИмпортер.ИНН КАК П000020000104,
	|	ВозвратыОборот.ПроизводительИмпортер.КПП КАК П000020000105,
	|	ВЫРАЗИТЬ(ВозвратыОборот.Контрагент.НаименованиеПолное КАК СТРОКА(200)) КАК П000020000106,
	|	ВЫРАЗИТЬ(КонтактнаяИнформация.Представление КАК СТРОКА(100)) КАК П000020000107,
	|	ВозвратыОборот.Контрагент.ИНН КАК П000020000108,
	|	ВозвратыОборот.Контрагент.КПП КАК П000020000109,
	|	"""" КАК П000020000110,
	|	"""" КАК П000020000111,
	|	"""" КАК П000020000113,
	|	"""" КАК П000020000114,
	|	"""" КАК П000020000115,
	|	СУММА(ВозвратыОборот.КоличествоОборотКт * СведенияОбАлкогольнойПродукции.КоэффПересчетаДал) КАК П000020000116,
	|	ВозвратыОборот.Регистратор.Дата КАК П000020000117,
	|	ВозвратыОборот.Регистратор.Номер КАК П000020000118,
	|	"""" КАК П000020000119,
	|	СУММА(ВозвратыОборот.КоличествоОборотКт * СведенияОбАлкогольнойПродукции.КоэффПересчетаДал) КАК П000020000120,
	|	ВозвратыОборот.Контрагент КАК Контрагент,
	|	ВозвратыОборот.Подразделение,
	|	ВозвратыОборот.Организация
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|		КОНЕЦ
	|	ИЗ
	|		РегистрБухгалтерии.Хозрасчетный.Обороты(&ДатаНач, &ДатаКон, Регистратор, Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ТМЦпринятыеНаОтветственноеХранение), &ВидСубконто, Организация В (&СписокОрганизаций), , ) КАК ХозрасчетныйОбороты
	|	ГДЕ
	|		ХозрасчетныйОбороты.КоличествоОборотКт > 0) КАК ВозвратыОборот
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КонтактнаяИнформация КАК КонтактнаяИнформация
	|		ПО (КонтактнаяИнформация.Вид = &ЮрАдрес)
	|			И ВозвратыОборот.Контрагент = КонтактнаяИнформация.Объект
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СведенияОбАлкогольнойПродукции КАК СведенияОбАлкогольнойПродукции
	|		ПО ВозвратыОборот.Номенклатура = СведенияОбАлкогольнойПродукции.Номенклатура
	|
	|СГРУППИРОВАТЬ ПО
	|	СведенияОбАлкогольнойПродукции.КодВида169,
	|	ВЫРАЗИТЬ(КонтактнаяИнформация.Представление КАК СТРОКА(100)),
	|	СведенияОбАлкогольнойПродукции.НаименованиеВида169,
	|	ВозвратыОборот.Регистратор.Номер,
	|	ВозвратыОборот.Регистратор.Дата,
	|	ВозвратыОборот.Регистратор,
	|	ВозвратыОборот.Подразделение,
	|	ВозвратыОборот.ПроизводительИмпортер.ИНН,
	|	ВозвратыОборот.ПроизводительИмпортер.КПП,
	|	ВозвратыОборот.Контрагент.ИНН,
	|	ВозвратыОборот.Контрагент.КПП,
	|	ВозвратыОборот.Контрагент,
	|	ВозвратыОборот.Организация,
	|	ВЫРАЗИТЬ(ВозвратыОборот.ПроизводительИмпортер.НаименованиеПолное КАК СТРОКА(200)),
	|	ВЫРАЗИТЬ(ВозвратыОборот.Контрагент.НаименованиеПолное КАК СТРОКА(200))
	|
	|УПОРЯДОЧИТЬ ПО
	|	П000020000102,
	|	П000020000106";
#КонецОбласти
	...
	Для Каждого СтрокаЗапроса Из ТаблицаЗапроса Цикл
		...
		КонецЕсли;
	КонецЦикла;
	СтрокаСортировки="П000020000102, П000020000103, П000020000106, П000020000117, П000020000118";
	ТаблицаЗапроса.Сортировать(СтрокаСортировки);
...
Функция ПолучитьСведенияОбАдресеКонтрагнета(Контрагент,СтрокаОшибок)
	
	#Область Проверка_ИНН_и_КПП //14.10.2016 10:55:42
	...
	Иначе
		Если СтрДлина(Контрагент.ИНН)<>10 Тогда
			СтрокаОшибок=СтрокаОшибок+Символы.ПС+"-Длина ИНН меньше требуемой";
		КонецЕсли;
		Если СтрДлина(Контрагент.КПП)<>9 Тогда
			СтрокаОшибок=СтрокаОшибок+Символы.ПС+"-Длина КПП меньше требуемой";
		КонецЕсли;
	КонецЕсли;
#КонецОбласти

	СтруктураАдреса=ИнициализацияСтруктурыСведенияОПоставщике();
	Запрос = Новый Запрос;
...
Функция ИнициализацияСтруктурыСведенияОПоставщике()
