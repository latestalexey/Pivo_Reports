...
Процедура ЗаполнитьАвтоБухгалтерия(СписокОрганизаций)
	...
	ТаблицаЗаполнения13.Колонки.Добавить("П000010000313", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14, 4));
	ТаблицаЗаполнения13.Колонки.Добавить("П000010000318", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14, 4));
	...
	Для Каждого СтрокаЗапроса Из ТаблицаЗапроса Цикл
		...
		СерияНомер = СокрЛП(СтрокаЗаполнения.П000020000192);
		ПозицияРазделителя = Найти(СерияНомер, ",");
		Если ПозицияРазделителя = 0 Тогда
			ПозицияРазделителя = Найти(СерияНомер, " ");
		КонецЕсли;
		Если ПозицияРазделителя <> 0 Тогда
			СтрокаЗаполнения.П000020000191 = СокрЛП(Лев(СерияНомер, ПозицияРазделителя - 1));
			СтрокаЗаполнения.П000020000192 = СокрЛП(Сред(СерияНомер, ПозицияРазделителя + 1));
		КонецЕсли;
...
Процедура ЗаполнитьАвтоБухгалтерияКОРП(СписокОрганизаций)
	...
	|		КОНЕЦ) КАК П000010000112,
	|	СУММА(ВЫБОР
	|			КОГДА ХозрасчетныйОбороты.КорСчет В (&Счет41) ИЛИ ХозрасчетныйОбороты.КорСчет В (&Счет79)
	|				ТОГДА ХозрасчетныйОбороты.КоличествоОборотДт * АлкогольнаяПродукция.КоэффПересчетаДал
	|			ИНАЧЕ 0
	...
	Для Каждого СтрокаЗапроса Из ТаблицаЗапроса Цикл
		...
		Если СтрокаЗапроса.П000010000113 > 0 Тогда
			Если БухгалтерскийУчет.ЭтоСубСчет(СтрокаЗапроса.КорСчет, ПланыСчетов.Хозрасчетный.ВнутрихозяйственныеРасчеты) Тогда
				Продолжить;
			КонецЕсли;
	...
	ТаблицаЗапроса.Сортировать("СсылкаПодразделение, П000020000101, П000020000102, П000020000103, П000020000113, П000020000115, П000020000106");
	...
	Для Каждого Подразделение Из ДеревоПоПодразделениям.Строки Цикл
		...
		Для Каждого СтрокаПодразделения Из Подразделение.Строки Цикл
			...
			СерияНомер = СокрЛП(СтрокаЗаполнения.П000020000392);
			ПозицияРазделителя = Найти(СерияНомер, ",");
			Если ПозицияРазделителя = 0 Тогда
				ПозицияРазделителя = Найти(СерияНомер, " ");
			КонецЕсли;
			Если ПозицияРазделителя <> 0 Тогда
				СтрокаЗаполнения.П000020000391 = СокрЛП(Лев(СерияНомер, ПозицияРазделителя - 1));
				СтрокаЗаполнения.П000020000392 = СокрЛП(Сред(СерияНомер, ПозицияРазделителя + 1));
			КонецЕсли;
...
Процедура ЗаполнитьАвтоУПП(СписокОрганизаций)
	...
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО 
	|	КОНЕЦ КАК П000020000114,
	|	ХозрасчетныйОбороты.Организация КАК Организация,
	|	СведенияОбАлкогольнойПродукции.ВидЛицензии,
	|	ВЫБОР КОГДА (НЕ ТоварыОрганизаций.СерияНоменклатуры ЕСТЬ NULL) ТОГДА ТоварыОрганизаций.СерияНоменклатуры.НомерГТД КОНЕЦ КАК П000020000115,
	|	ХозрасчетныйОбороты.СубконтоКт1,
	|	СУММА(ВЫБОР 
...
Процедура УстановитьДоступностьПолейРаздел1()
	...
	Пока ПТД_Раздел1.Области.Найти("П000010000101_" + Постфикс) <> Неопределено Цикл
		...
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000114_" + Постфикс, ВариантАвторасчета);
		...
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000117_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000119_" + Постфикс, ВариантАвторасчета);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000120_" + Постфикс, ВариантАвторасчета);
	...
	ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000219", ВариантАвторасчета);
	ИзменитьВариантЗаполненияОбласти(ПТД_Раздел1, "П000010000220", ВариантАвторасчета);
...
Процедура УстановитьДоступностьПолейРаздел2()
	...
	Пока ПТД_Раздел2.Области.Найти("П000020000101_" + Постфикс) <> Неопределено Цикл
		...
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел2, "П000020000191_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел2, "П000020000192_" + Постфикс, ВариантАвтозаполнения);
		...
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел2, "П000020000113_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел2, "П000020000114_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел2, "П000020000115_" + Постфикс, ВариантАвтозаполнения);
		ИзменитьВариантЗаполненияОбласти(ПТД_Раздел2, "П000020000116_" + Постфикс, ВариантАвтозаполнения);
		
		НомерСтроки = НомерСтроки + 1;
		Постфикс = Формат(НомерСтроки, "ЧГ=0");
	КонецЦикла;
	
	ИзменитьВариантЗаполненияОбласти(ПТД_Раздел2, "П000020000216", ВариантАвторасчета);
...
Процедура СформироватьСпискиВыбора()
	...
	МакетСоставаПоказателей = ПолучитьМакет("Списки2012Кв3");
...
Функция ОбновитьВариантыЗаполнения()
	...
	Для Инд = 0 По МакетСоставаПоказателей.Области.Количество() - 1 Цикл
		...
		Для Ном = ТекОбласть.Верх По ТекОбласть.Низ Цикл
			...
			Если ИдКонфигурации = "БГУ" И СокрЛП(стрВариантЗаполнения) = "3" Тогда
				стрВариантЗаполнения = "0";
			КонецЕсли;
...
Процедура РасчетРаздел1(Знач МестоРазмещенияДанных = Неопределено)
	...
	РасчетРаздел1Таблица1(МестоРазмещенияДанных);
	РасчетРаздел1Таблица2(МестоРазмещенияДанных);
	
КонецПроцедуры

Процедура РасчетРаздел1Таблица1(МестоРазмещенияДанных)
	...
	Для Инд = 1 По КоличествоСтрок Цикл
		...
		ПоместитьДанныеПоМестуРазмещения(МестоРазмещенияДанных, КодСекции + "19" + ПостфиксСтроки, ЗначениеГрафы19);
		...
		ПоместитьДанныеПоМестуРазмещения(МестоРазмещенияДанных, КодСекции + "20" + ПостфиксСтроки, ЗначениеГрафы20);
...
Процедура РасчетРаздел1Таблица2(МестоРазмещенияДанных)
...
Процедура РасчетРаздел2(Знач МестоРазмещенияДанных = Неопределено)
	...
	РасчетРаздел2Таблица1(МестоРазмещенияДанных);
	РасчетРаздел2Таблица2(МестоРазмещенияДанных);
	
КонецПроцедуры

Процедура РасчетРаздел2Таблица1(МестоРазмещенияДанных)
...
Процедура РасчетРаздел2Таблица2(МестоРазмещенияДанных)
...
Процедура ИнициализироватьПоляВводаТабличныхДокументов()
	...
	ПТД_Раздел1.Области.П000010000104_1.ЭлементУправления.Маска = МаскаИНН10;
	ПТД_Раздел1.Области.П000010000304_1.ЭлементУправления.Маска = МаскаИНН10;
	
	ПТД_Раздел2.Области.П000020000104_1.ЭлементУправления.Маска = МаскаИНН10;
	ПТД_Раздел2.Области.П000020000304_1.ЭлементУправления.Маска = МаскаИНН10;
...
Процедура ПроверитьЗаполнениеЛицензии(Параметры, СтатусПроверки)
	...
	Для Каждого ЛистРаздела Из ДанныеРаздела Цикл
		...
		Если НЕ ЗаполненЛистЛицензии(ДанныеЛиста) Тогда
			Продолжить;
		КонецЕсли;
		...
		ИначеЕсли НЕ ЗначениеЗаполнено(ВидДеятельности) Тогда
			...
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Лицензии", "П000000000105", НомерЛиста);
		ИначеЕсли НЕ ЗначениеСодержитсяВСписке(КодВидаДеятельности, "14") Тогда
			ТекстОписанияОшибки = "Неверно задан вид деятельности по лицензии на листе " + НомерЛистаСтроковый + " раздела лицензий!";
			СообщитьОбОшибкеЗаполнения(СтатусПроверки, ТекстОписанияОшибки, "Лицензии", "П000000000105", НомерЛиста);
		КонецЕсли;
		
	КонецЦикла;
	
	ТекстОписанияОшибки = "Не заполнен ни один лист раздела Лицензии!";
...
Процедура ПроверитьЗаполнениеРаздел1(Параметры, СтатусПроверки)
	...
	Для Каждого ЛистРаздела Из ДанныеРаздела Цикл
		...
		Если ТипЗнч(ЛистРаздела.ДополнительныеДанные) = Тип("Структура") Тогда
			ЛистРаздела.ДополнительныеДанные.Свойство(мИдГруппы11, ДополнительныеДанные);
		КонецЕсли;
		ПроверитьЗаполнениеТаблицыРаздел1(СтатусПроверки, ДанныеДопСтрок, ДополнительныеДанные, НомерЛиста);
		
		ЛистРаздела.ДанныеДопСтрок.Свойство(мИдГруппы12, ДанныеДопСтрок);
		Если ТипЗнч(ЛистРаздела.ДополнительныеДанные) = Тип("Структура") Тогда
			ЛистРаздела.ДополнительныеДанные.Свойство(мИдГруппы12, ДополнительныеДанные);
		КонецЕсли;
		ПроверитьЗаполнениеТаблицыРаздел1(СтатусПроверки, ДанныеДопСтрок, ДополнительныеДанные, НомерЛиста);
...
Процедура ПроверитьЗаполнениеРаздел2(Параметры, СтатусПроверки)
	...
	Для Каждого ЛистРаздела Из ДанныеРаздела Цикл
		...
		Если ТипЗнч(ЛистРаздела.ДополнительныеДанные) = Тип("Структура") Тогда
			ЛистРаздела.ДополнительныеДанные.Свойство(мИдГруппы21, ДополнительныеДанные);
		КонецЕсли;
		ПроверитьЗаполнениеТаблицыРаздел2(СтатусПроверки, ДанныеДопСтрок, ДополнительныеДанные, НомерЛиста);
		
		ЛистРаздела.ДанныеДопСтрок.Свойство(мИдГруппы22, ДанныеДопСтрок);
		Если ТипЗнч(ЛистРаздела.ДополнительныеДанные) = Тип("Структура") Тогда
			ЛистРаздела.ДополнительныеДанные.Свойство(мИдГруппы22, ДополнительныеДанные);
		КонецЕсли;
		ПроверитьЗаполнениеТаблицыРаздел2(СтатусПроверки, ДанныеДопСтрок, ДополнительныеДанные, НомерЛиста);
...
Функция ЗаполненЛистЛицензии(ДанныеЛиста)
	
	Заполнен = ЗначениеЗаполнено(ДанныеЛиста.П000000000101)
	...
	Для Каждого Показатель Из ДанныеЛиста Цикл
		ИмяПоказателя = Показатель.Ключ;
		Если Найти(ИмяПоказателя, "П0000000002") = 1 Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Имясобытия = "Изменение статуса отправки регламентированного отчета" И ЗначениеЗаполнено(Параметр) И мСохраненныйДок <> Неопределено И Источник = мСохраненныйДок.Ссылка Тогда
		...
		Если мСохраненныйДок <> Неопределено Тогда
			РеквизитыОтправки = РегламентированнаяОтчетность.ПолучитьРеквизитыОтправкиФСРАР(мСохраненныйДок.Ссылка);
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = ПолучитьМакет("СхемаВыгрузки420");
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель, КорневойУзел, Параметры)
	...
	ПроизводителиИмпортеры.Колонки.Добавить("П000000000004"); // Наименование производителя/импортёра
	...
	Для Каждого Страница Из мДанныеОтчета.Раздел1 Цикл
		...
		Для Каждого СтраницаТабл2 Из мДанныеОтчета.Раздел2 Цикл
			Если мДанныеОтчета.Раздел2.Колонки.Найти("ДополнительныеДанные") = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Если НЕ СтраницаТабл2.ДополнительныеДанные.Свойство("СведенияОбОП") Тогда
				Продолжить;
			КонецЕсли;
			
			Если СтраницаТабл2.ДополнительныеДанные.СведенияОбОП.КПП = КПП
			   И СтраницаТабл2.ДополнительныеДанные.СведенияОбОП.Наименование = НазваниеОрганизации Тогда
...
Процедура ЗаполнитьСправочникиДекларации(УзелСправочники, ДанныеВыгрузки)
	...
	УдалитьУзел(ПрототипУзла_Поставщики);
...
Процедура КоманднаяПанельФормыОтправить(Кнопка)
	...
	КодРегионаВФорме = РегламентированнаяОтчетность.роВернутьЗначениеПараметраСЛиста(ЭтаФорма, "Титульный", "КодРегиона");
	
	РегламентированнаяОтчетность.ОтправитьОбъектВФСРАР(мСохраненныйДок.Ссылка, Справочники.ВидыОтправляемыхДокументов.РозничнаяПродажаАлкогольнойИСпиртосодержащейПродукции, КодРегионаВФорме);
...
Процедура КоманднаяПанельФормыПолучитьРезультат(Кнопка)
	...
	Если РегламентированнаяОтчетность.ОбновитьРезультатОтправкиФСРАР(мСохраненныйДок.Ссылка, Справочники.ВидыОтправляемыхДокументов.РозничнаяПродажаАлкогольнойИСпиртосодержащейПродукции) Тогда
...
Процедура КоманднаяПанельФормыПоказатьОтправки(Кнопка)
	...
	РегламентированнаяОтчетность.ПоказатьОтправкиФСРАР(СсылкаФСРАР);
...
Процедура КнопкаОбновитьСостояниеОтправкиНажатие(Элемент)
	...
	Если РеквизитыОтправки <> Неопределено Тогда
		Если РеквизитыОтправки.СтатусОтправки = Перечисления.СтатусыОтправки.Отправлен Тогда
			РегламентированнаяОтчетность.ОбновитьРезультатОтправкиФСРАР(мСохраненныйДок.Ссылка, Справочники.ВидыОтправляемыхДокументов.РозничнаяПродажаАлкогольнойИСпиртосодержащейПродукции);
		КонецЕсли;
	КонецЕсли;
...
Функция ВыгрузитьПакет()
	...
	Если НЕ РегламентированнаяОтчетность.ИнициализироватьВнешнююКомпоненту() Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	ПараметрыФормированияПакета = ПолучитьПараметрыФормированияПакета();
	Если НЕ ЗначениеЗаполнено(ПараметрыФормированияПакета) Тогда
		Возврат Неопределено;
	КонецЕсли;
	СертификатАбонента = ПараметрыФормированияПакета.СертификатАбонента;
	СертификатСубъектаРФ = ПараметрыФормированияПакета.СертификатСубъектаРФ;
	СертификатФСРАР = ПараметрыФормированияПакета.СертификатФСРАР;
	...
	ДокВыгрузки.УстановитьВремя(РежимАвтоВремя.ТекущееИлиПоследним);
	ПолноеИмяФайлаВыгрузки = ДокВыгрузки.ПолучитьФорму().ВыгрузитьОтчет(мСохраненныйДок.Ссылка, Ложь, Ложь);
	...
	// подписываем
	Состояние("Подписание файла выгрузки ...");
	Попытка
		Крипт.PKCS7ПодписатьСДанными(СертификатАбонента.СерийныйНомер, СертификатАбонента.Поставщик,
			ПолноеИмяФайлаВыгрузки, ПолноеИмяФайлаВыгрузки + ".sig");
	Исключение
		УдалитьВременныйФайл(ВременныйКаталог);
		Сообщить("Не удалось подписать файл выгрузки:
				|" + ИнформацияОбОшибке().Описание, СтатусСообщения.Важное);
		Возврат Неопределено;
	КонецПопытки;
	
	// упаковываем
	Состояние("Упаковка файла выгрузки ...");
	Попытка
		ZipФайл = Новый ЗаписьZipФайла(ПолноеИмяФайлаВыгрузки + ".sig.zip");
		ZipФайл.Добавить(ПолноеИмяФайлаВыгрузки + ".sig");
		ZipФайл.Записать();
	Исключение
		УдалитьВременныйФайл(ВременныйКаталог);
		Сообщить("Не удалось упаковать файл выгрузки:
				|" + ИнформацияОбОшибке().Описание, СтатусСообщения.Важное);
		Возврат Неопределено;
	КонецПопытки;
	
	// шифруем
	Состояние("Шифрование файла выгрузки ...");
	ИмяФайлаЗашифрованногоПакета = КаталогВыгрузки + ИмяФайлаВыгрузки + ".sig.zip.enc";
	МассивСерийныхНомеров = Новый Массив;
	МассивПоставщиков = Новый Массив;
	МассивСерийныхНомеров.Добавить(СертификатСубъектаРФ.СерийныйНомер);
	МассивПоставщиков.Добавить(СертификатСубъектаРФ.Поставщик);
	МассивСерийныхНомеров.Добавить(СертификатФСРАР.СерийныйНомер);
	МассивПоставщиков.Добавить(СертификатФСРАР.Поставщик);
	Попытка
		Крипт.PKCS7ЗашифроватьДляНескольких(ПолноеИмяФайлаВыгрузки + ".sig.zip", МассивСерийныхНомеров,
			МассивПоставщиков, ИмяФайлаЗашифрованногоПакета);
	Исключение
		УдалитьВременныйФайл(ВременныйКаталог);
		Сообщить("Не удалось зашифровать файл выгрузки:
				|" + ИнформацияОбОшибке().Описание, СтатусСообщения.Важное);
		Возврат Неопределено;
	КонецПопытки;
	...
	Возврат ИмяФайлаЗашифрованногоПакета;
...
Процедура ПоказатьПротоколТекущейОтправки()
	...
	РеквизитыОтправки = РегламентированнаяОтчетность.ПолучитьРеквизитыОтправкиФСРАР(мСохраненныйДок.Ссылка, Истина);
...
Функция ПолучитьПараметрыФормированияПакета()
	
	ФормаПараметров = ПолучитьФорму("ПараметрыФормированияПакета");
	ФормаПараметров.Организация = Организация;
...
СтруктураГруппы11.Колонки.Добавить("П000010000103", ОписаниеТиповСтрока254);
...
СтруктураГруппы11.Колонки.Добавить("П000010000119", ОписаниеТиповПоказательВДекалитрах);
...
СтруктураГруппы12.Колонки.Добавить("П000010000303", ОписаниеТиповСтрока254);
...
СтруктураГруппы12.Колонки.Добавить("П000010000319", ОписаниеТиповПоказательВДекалитрах);
...
СтруктураГруппы21.Колонки.Добавить("П000020000103", ОписаниеТиповСтрока254);
...
СтруктураГруппы21.Колонки.Добавить("П000020000106", ОписаниеТиповСтрока254);
...
СтруктураГруппы21.Колонки.Добавить("П000020000191", ОписаниеТиповСтрока254);
СтруктураГруппы21.Колонки.Добавить("П000020000192", ОписаниеТиповСтрока254);
СтруктураГруппы21.Колонки.Добавить("П000020000110", ОписаниеТиповДата);
СтруктураГруппы21.Колонки.Добавить("П000020000111", ОписаниеТиповДата);
СтруктураГруппы21.Колонки.Добавить("П000020000112", ОписаниеТиповСтрока254);
СтруктураГруппы21.Колонки.Добавить("П000020000113", ОписаниеТиповДата);
СтруктураГруппы21.Колонки.Добавить("П000020000114", ОписаниеТиповСтрока254);
СтруктураГруппы21.Колонки.Добавить("П000020000115", ОписаниеТиповСтрока254);
СтруктураГруппы21.Колонки.Добавить("П000020000116", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14, 4));
...
СтруктураГруппы22.Колонки.Добавить("П000020000303", ОписаниеТиповСтрока254);
...
СтруктураГруппы22.Колонки.Добавить("П000020000306", ОписаниеТиповСтрока254);
...
СтруктураГруппы22.Колонки.Добавить("П000020000391", ОписаниеТиповСтрока254);
СтруктураГруппы22.Колонки.Добавить("П000020000392", ОписаниеТиповСтрока254);
СтруктураГруппы22.Колонки.Добавить("П000020000310", ОписаниеТиповДата);
СтруктураГруппы22.Колонки.Добавить("П000020000311", ОписаниеТиповДата);
СтруктураГруппы22.Колонки.Добавить("П000020000312", ОписаниеТиповСтрока254);
СтруктураГруппы22.Колонки.Добавить("П000020000313", ОписаниеТиповДата);
СтруктураГруппы22.Колонки.Добавить("П000020000314", ОписаниеТиповСтрока254);
СтруктураГруппы22.Колонки.Добавить("П000020000315", ОписаниеТиповСтрока254);
СтруктураГруппы22.Колонки.Добавить("П000020000316", ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(14, 4));
